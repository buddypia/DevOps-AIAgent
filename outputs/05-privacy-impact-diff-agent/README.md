# Privacy Impact Diff Agent

Finds personal-data collection, storage, logging, and transfer changes.

## 概要

Privacy Impact Diff Agent は、DevOps x AI Agent Hackathon 向けの `privacy diff reviewer` です。Reviews PR evidence for privacy-impact changes and creates a concise mitigation checklist.

MVPでは、スコープを絞って次の体験を作ります。A web app that flags PII paths, retention questions, log exposure, and external sharing risks.
...
- 判断ラベル: `CLEAR` / `REVIEW` / `BLOCK PRIVACY`
...
Privacy Impact Diff Agent は、その中でも次の運用判断にフォーカスします。
...
`privacy diff reviewer` の仕事は影響範囲が大きく、ひとつの誤ったデプロイ判断、ロールバック判断、初動対応、確認漏れが、数時間の調査やユーザー信頼の低下につながります。チームには、次のような軽量な支援役が必要です。

- 雑多な運用情報を素早く読む
- 判断を `CLEAR` / `REVIEW` / `BLOCK PRIVACY` のように明確化する
- 判断の根拠を提示する
...
- 人間の担当者は、曖昧な要約ではなく `CLEAR` / `REVIEW` / `BLOCK PRIVACY` の具体的な判断を受け取れます。
...
エージェントの人格は `privacy diff reviewer` です。プロンプトは、プロジェクトの目的と次のフォーカス領域に制約しています。
...
- `decision`: `CLEAR` / `REVIEW` / `BLOCK PRIVACY` のいずれか
...
- Target: `https://github.com/example/crm/pull/92`
- Context: The PR adds customer success notes and exports support metadata to analytics.
- Signals: Diff: new fields customer_email, freeform_note, account_health. Logs: debug statement includes customer payload. Destination: analytics topic support-events. Retention doc: not updated. Tests: schema test added.
...
```bash
gcloud run deploy privacy-impact-diff-agent \
  --source . \
...
2. Runボタンで分析を実行します。
3. 生の運用情報が、どのように `CLEAR` / `REVIEW` / `BLOCK PRIVACY` の判断へ変換されるか説明します。
...
- [操作マニュアル](./docs/manual.html): Privacy Impact Diff Agent の仮説、操作手順、判断の読み方、デモ確認項目をHTML図解で確認できます。
