# Privacy Impact Diff Agent Terraform Deploy Guide
...
![Privacy Impact Diff Agent architecture](./architecture.svg)
...
```bash
cd outputs/05-privacy-impact-diff-agent/infra/terraform
terraform init
terraform apply \
...
```bash
printf "%s" "$GEMINI_API_KEY" | gcloud secrets create privacy-impact-diff-agent-gemini-api-key \
...
terraform apply \
  -var project_id="$GOOGLE_CLOUD_PROJECT" \
  -var existing_gemini_api_secret_id="privacy-impact-diff-agent-gemini-api-key" \
...
Open `$SERVICE_URL`, run the sample scenario, and confirm a CLEAR / REVIEW / BLOCK PRIVACY decision is rendered.
