# Rollback Concierge Terraform Deploy Guide
...
![Rollback Concierge architecture](./architecture.svg)
...
```bash
cd outputs/13-rollback-concierge/infra/terraform
terraform init
terraform apply \
...
```bash
printf "%s" "$GEMINI_API_KEY" | gcloud secrets create rollback-concierge-gemini-api-key \
...
terraform apply \
  -var project_id="$GOOGLE_CLOUD_PROJECT" \
  -var existing_gemini_api_secret_id="rollback-concierge-gemini-api-key" \
...
Open `$SERVICE_URL`, run the sample scenario, and confirm a ROLLBACK READY / MANUAL CHECK / DO NOT ROLLBACK decision is rendered.
