...
Builds a safer rollback order across revisions and data changes.

## 概要

Rollback Concierge は、DevOps x AI Agent Hackathon 向けの `rollback sequencing agent` です。Looks at Cloud Run revisions, DB changes, and operational state to propose rollback sequencing.

MVPでは、スコープを絞って次の体験を作ります。A rollback planner that lists the safest order, verification checks, and irreversible steps.
...
- 判断ラベル: `ROLLBACK READY` / `MANUAL CHECK` / `DO NOT ROLLBACK`
...
Rollback Concierge は、その中でも次の運用判断にフォーカスします。
...
`rollback sequencing agent` の仕事は影響範囲が大きく、ひとつの誤ったデプロイ判断、ロールバック判断、初動対応、確認漏れが、数時間の調査やユーザー信頼の低下につながります。チームには、次のような軽量な支援役が必要です。

- 雑多な運用情報を素早く読む
- 判断を `ROLLBACK READY` / `MANUAL CHECK` / `DO NOT ROLLBACK` のように明確化する
- 判断の根拠を提示する
...
- 人間の担当者は、曖昧な要約ではなく `ROLLBACK READY` / `MANUAL CHECK` / `DO NOT ROLLBACK` の具体的な判断を受け取れます。
...
エージェントの人格は `rollback sequencing agent` です。プロンプトは、プロジェクトの目的と次のフォーカス領域に制約しています。
...
- `decision`: `ROLLBACK READY` / `MANUAL CHECK` / `DO NOT ROLLBACK` のいずれか
...
- Target: `checkout-api rollback plan`
- Context: New release changed retry handling and added nullable DB column payment_attempt_source.
- Signals: Cloud Run previous revision healthy. DB migration add-column only, no backfill. Feature flag can disable retry_v2. Logs show timeout spike. No irreversible delete migration. Verification SQL exists for failed checkout count.
...
```bash
gcloud run deploy rollback-concierge \
  --source . \
...
2. Runボタンで分析を実行します。
3. 生の運用情報が、どのように `ROLLBACK READY` / `MANUAL CHECK` / `DO NOT ROLLBACK` の判断へ変換されるか説明します。
...
- [操作マニュアル](./docs/manual.html): Rollback Concierge の仮説、操作手順、判断の読み方、デモ確認項目をHTML図解で確認できます。
