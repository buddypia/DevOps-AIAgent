# AI Exploratory Tester Terraform Deploy Guide
...
![AI Exploratory Tester architecture](./architecture.svg)
...
```bash
cd outputs/16-ai-exploratory-tester/infra/terraform
terraform init
terraform apply \
...
```bash
printf "%s" "$GEMINI_API_KEY" | gcloud secrets create ai-exploratory-tester-gemini-api-key \
...
terraform apply \
  -var project_id="$GOOGLE_CLOUD_PROJECT" \
  -var existing_gemini_api_secret_id="ai-exploratory-tester-gemini-api-key" \
...
Open `$SERVICE_URL`, run the sample scenario, and confirm a TEST NOW / EXPLORE MORE / BLOCK RELEASE decision is rendered.
