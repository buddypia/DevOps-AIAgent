# AI Exploratory Tester

Turns a target URL and notes into exploration charters and bug reports.

## 概要

AI Exploratory Tester は、DevOps x AI Agent Hackathon 向けの `exploratory QA agent` です。Plans exploratory testing routes, identifies likely broken flows, and drafts reproducible issues.

MVPでは、スコープを絞って次の体験を作ります。A QA workbench that creates test charters, expected observations, and bug report skeletons.
...
- 判断ラベル: `TEST NOW` / `EXPLORE MORE` / `BLOCK RELEASE`
...
AI Exploratory Tester は、その中でも次の運用判断にフォーカスします。
...
`exploratory QA agent` の仕事は影響範囲が大きく、ひとつの誤ったデプロイ判断、ロールバック判断、初動対応、確認漏れが、数時間の調査やユーザー信頼の低下につながります。チームには、次のような軽量な支援役が必要です。

- 雑多な運用情報を素早く読む
- 判断を `TEST NOW` / `EXPLORE MORE` / `BLOCK RELEASE` のように明確化する
- 判断の根拠を提示する
...
- 人間の担当者は、曖昧な要約ではなく `TEST NOW` / `EXPLORE MORE` / `BLOCK RELEASE` の具体的な判断を受け取れます。
...
エージェントの人格は `exploratory QA agent` です。プロンプトは、プロジェクトの目的と次のフォーカス領域に制約しています。
...
- `decision`: `TEST NOW` / `EXPLORE MORE` / `BLOCK RELEASE` のいずれか
...
- Target: `https://staging.example.com/signup`
- Context: New onboarding flow includes plan selection, team invite, and billing setup. Release candidate is ready for exploratory QA.
- Signals: Known risk: long organization names, Japanese locale, failed card, back button from invite step. Existing e2e only covers happy path. Recent UI diff changed validation and empty states.
...
```bash
gcloud run deploy ai-exploratory-tester \
  --source . \
...
2. Runボタンで分析を実行します。
3. 生の運用情報が、どのように `TEST NOW` / `EXPLORE MORE` / `BLOCK RELEASE` の判断へ変換されるか説明します。
...
- [操作マニュアル](./docs/manual.html): AI Exploratory Tester の仮説、操作手順、判断の読み方、デモ確認項目をHTML図解で確認できます。
