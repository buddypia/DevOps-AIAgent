...
Chooses the three decisions humans should make now.

## 概要

Decision Fatigue Reducer は、DevOps x AI Agent Hackathon 向けの `decision prioritization agent` です。Reads crowded PR and issue context to rank the decisions that unblock delivery.

MVPでは、スコープを絞って次の体験を作ります。A decision queue that groups noisy work into now, later, and delegate recommendations.
...
- 判断ラベル: `DECIDE THREE` / `DEFER` / `ESCALATE`
...
Decision Fatigue Reducer は、その中でも次の運用判断にフォーカスします。
...
`decision prioritization agent` の仕事は影響範囲が大きく、ひとつの誤ったデプロイ判断、ロールバック判断、初動対応、確認漏れが、数時間の調査やユーザー信頼の低下につながります。チームには、次のような軽量な支援役が必要です。

- 雑多な運用情報を素早く読む
- 判断を `DECIDE THREE` / `DEFER` / `ESCALATE` のように明確化する
- 判断の根拠を提示する
...
- 人間の担当者は、曖昧な要約ではなく `DECIDE THREE` / `DEFER` / `ESCALATE` の具体的な判断を受け取れます。
...
エージェントの人格は `decision prioritization agent` です。プロンプトは、プロジェクトの目的と次のフォーカス領域に制約しています。
...
- `decision`: `DECIDE THREE` / `DEFER` / `ESCALATE` のいずれか
...
- Target: `sprint release queue`
- Context: Team has 18 open PRs, 9 release questions, and two hours before branch freeze.
- Signals: PRs: billing copy, retry policy, mobile deep link, analytics schema. CI failures on retry policy. Product question about beta scope. Security question about logging customer email. Two reviewers overloaded.
...
```bash
gcloud run deploy decision-fatigue-reducer \
  --source . \
...
2. Runボタンで分析を実行します。
3. 生の運用情報が、どのように `DECIDE THREE` / `DEFER` / `ESCALATE` の判断へ変換されるか説明します。
...
- [操作マニュアル](./docs/manual.html): Decision Fatigue Reducer の仮説、操作手順、判断の読み方、デモ確認項目をHTML図解で確認できます。
