# Decision Fatigue Reducer Terraform Deploy Guide
...
![Decision Fatigue Reducer architecture](./architecture.svg)
...
```bash
cd outputs/17-decision-fatigue-reducer/infra/terraform
terraform init
terraform apply \
...
```bash
printf "%s" "$GEMINI_API_KEY" | gcloud secrets create decision-fatigue-reducer-gemini-api-key \
...
terraform apply \
  -var project_id="$GOOGLE_CLOUD_PROJECT" \
  -var existing_gemini_api_secret_id="decision-fatigue-reducer-gemini-api-key" \
...
Open `$SERVICE_URL`, run the sample scenario, and confirm a DECIDE THREE / DEFER / ESCALATE decision is rendered.
