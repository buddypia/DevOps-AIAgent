# Chaos Drill Agent Terraform Deploy Guide
...
![Chaos Drill Agent architecture](./architecture.svg)
...
```bash
cd outputs/19-chaos-drill-agent/infra/terraform
terraform init
terraform apply \
...
```bash
printf "%s" "$GEMINI_API_KEY" | gcloud secrets create chaos-drill-agent-gemini-api-key \
...
terraform apply \
  -var project_id="$GOOGLE_CLOUD_PROJECT" \
  -var existing_gemini_api_secret_id="chaos-drill-agent-gemini-api-key" \
...
Open `$SERVICE_URL`, run the sample scenario, and confirm a RUN DRILL / TIGHTEN GUARDRAILS / DO NOT RUN decision is rendered.
