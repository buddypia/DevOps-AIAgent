...
Designs a safe drill and scores detection, judgment, and recovery.

## 概要

Chaos Drill Agent は、DevOps x AI Agent Hackathon 向けの `chaos exercise conductor` です。Creates controlled failure drills, expected signals, scoring rubrics, and recovery evaluation.

MVPでは、スコープを絞って次の体験を作ります。A chaos drill planner that outputs scenario, guardrails, scoring, and post-drill improvements.
...
- 判断ラベル: `RUN DRILL` / `TIGHTEN GUARDRAILS` / `DO NOT RUN`
...
Chaos Drill Agent は、その中でも次の運用判断にフォーカスします。
...
`chaos exercise conductor` の仕事は影響範囲が大きく、ひとつの誤ったデプロイ判断、ロールバック判断、初動対応、確認漏れが、数時間の調査やユーザー信頼の低下につながります。チームには、次のような軽量な支援役が必要です。

- 雑多な運用情報を素早く読む
- 判断を `RUN DRILL` / `TIGHTEN GUARDRAILS` / `DO NOT RUN` のように明確化する
- 判断の根拠を提示する
...
- 人間の担当者は、曖昧な要約ではなく `RUN DRILL` / `TIGHTEN GUARDRAILS` / `DO NOT RUN` の具体的な判断を受け取れます。
...
エージェントの人格は `chaos exercise conductor` です。プロンプトは、プロジェクトの目的と次のフォーカス領域に制約しています。
...
- `decision`: `RUN DRILL` / `TIGHTEN GUARDRAILS` / `DO NOT RUN` のいずれか
...
- Target: `checkout timeout chaos drill`
- Context: Team wants a 10-minute staging drill that simulates payment provider latency without affecting production.
- Signals: Staging only. Proposed injection: add 900ms latency to payment sandbox for 20% requests. Monitors: checkout p95, retry warnings, synthetic checkout. Guardrails: stop at 2% 5xx or p95 > 2s. Runbook owner available.
...
```bash
gcloud run deploy chaos-drill-agent \
  --source . \
...
2. Runボタンで分析を実行します。
3. 生の運用情報が、どのように `RUN DRILL` / `TIGHTEN GUARDRAILS` / `DO NOT RUN` の判断へ変換されるか説明します。
...
- [操作マニュアル](./docs/manual.html): Chaos Drill Agent の仮説、操作手順、判断の読み方、デモ確認項目をHTML図解で確認できます。
