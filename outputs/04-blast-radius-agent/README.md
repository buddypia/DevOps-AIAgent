...
Maps a PR to affected screens, APIs, data, and runbooks.

## 概要

Blast Radius Agent は、DevOps x AI Agent Hackathon 向けの `change impact forecaster` です。Turns a PR diff and service notes into an operational impact forecast before release.

MVPでは、スコープを絞って次の体験を作ります。A PR impact analyzer that groups touched surfaces, test gaps, and watchpoints for reviewers.
...
- 判断ラベル: `LOW RADIUS` / `WATCH RADIUS` / `HIGH RADIUS`
...
Blast Radius Agent は、その中でも次の運用判断にフォーカスします。
...
`change impact forecaster` の仕事は影響範囲が大きく、ひとつの誤ったデプロイ判断、ロールバック判断、初動対応、確認漏れが、数時間の調査やユーザー信頼の低下につながります。チームには、次のような軽量な支援役が必要です。

- 雑多な運用情報を素早く読む
- 判断を `LOW RADIUS` / `WATCH RADIUS` / `HIGH RADIUS` のように明確化する
- 判断の根拠を提示する
...
- 人間の担当者は、曖昧な要約ではなく `LOW RADIUS` / `WATCH RADIUS` / `HIGH RADIUS` の具体的な判断を受け取れます。
...
エージェントの人格は `change impact forecaster` です。プロンプトは、プロジェクトの目的と次のフォーカス領域に制約しています。
...
- `decision`: `LOW RADIUS` / `WATCH RADIUS` / `HIGH RADIUS` のいずれか
...
- Target: `https://github.com/example/app/pull/211`
- Context: PR touches subscription billing, invoice webhooks, and admin exports. Release owner needs an impact map.
- Signals: Diff: billing/plan.ts, webhooks/stripe.ts, admin/export.sql, invoice email copy. Tests: unit coverage added for plan upgrade only. Logs: webhook retries are already noisy. Runbook: billing rollback mentions old flag name.
...
```bash
gcloud run deploy blast-radius-agent \
  --source . \
...
2. Runボタンで分析を実行します。
3. 生の運用情報が、どのように `LOW RADIUS` / `WATCH RADIUS` / `HIGH RADIUS` の判断へ変換されるか説明します。
...
- [操作マニュアル](./docs/manual.html): Blast Radius Agent の仮説、操作手順、判断の読み方、デモ確認項目をHTML図解で確認できます。
