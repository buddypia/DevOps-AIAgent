# Blast Radius Agent Terraform Deploy Guide
...
![Blast Radius Agent architecture](./architecture.svg)
...
```bash
cd outputs/04-blast-radius-agent/infra/terraform
terraform init
terraform apply \
...
```bash
printf "%s" "$GEMINI_API_KEY" | gcloud secrets create blast-radius-agent-gemini-api-key \
...
terraform apply \
  -var project_id="$GOOGLE_CLOUD_PROJECT" \
  -var existing_gemini_api_secret_id="blast-radius-agent-gemini-api-key" \
...
Open `$SERVICE_URL`, run the sample scenario, and confirm a LOW RADIUS / WATCH RADIUS / HIGH RADIUS decision is rendered.
