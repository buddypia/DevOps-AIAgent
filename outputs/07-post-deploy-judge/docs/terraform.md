# Post Deploy Judge Terraform Deploy Guide
...
![Post Deploy Judge architecture](./architecture.svg)
...
```bash
cd outputs/07-post-deploy-judge/infra/terraform
terraform init
terraform apply \
...
```bash
printf "%s" "$GEMINI_API_KEY" | gcloud secrets create post-deploy-judge-gemini-api-key \
...
terraform apply \
  -var project_id="$GOOGLE_CLOUD_PROJECT" \
  -var existing_gemini_api_secret_id="post-deploy-judge-gemini-api-key" \
...
Open `$SERVICE_URL`, run the sample scenario, and confirm a SUCCESS / WATCH / ROLLBACK decision is rendered.
