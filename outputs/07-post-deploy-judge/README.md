...
Judges the first 15 minutes after deploy.

## 概要

Post Deploy Judge は、DevOps x AI Agent Hackathon 向けの `post-deployment decision agent` です。Analyzes post-deploy logs and metrics to call success, continued watch, or rollback.

MVPでは、スコープを絞って次の体験を作ります。A deployment observation board that explains the verdict and creates the next monitoring window.
...
Post Deploy Judge は、その中でも次の運用判断にフォーカスします。
...
`post-deployment decision agent` の仕事は影響範囲が大きく、ひとつの誤ったデプロイ判断、ロールバック判断、初動対応、確認漏れが、数時間の調査やユーザー信頼の低下につながります。チームには、次のような軽量な支援役が必要です。

- 雑多な運用情報を素早く読む
- 判断を `SUCCESS` / `WATCH` / `ROLLBACK` のように明確化する
- 判断の根拠を提示する
...
- 人間の担当者は、曖昧な要約ではなく `SUCCESS` / `WATCH` / `ROLLBACK` の具体的な判断を受け取れます。
...
エージェントの人格は `post-deployment decision agent` です。プロンプトは、プロジェクトの目的と次のフォーカス領域に制約しています。
...
- `decision`: `SUCCESS` / `WATCH` / `ROLLBACK` のいずれか
...
- Target: `checkout-api deploy 2026-05-13 10:00 JST`
- Context: Revision checkout-api-00153 reached 100% traffic 15 minutes ago. Release owner needs a go/no-go call.
- Signals: First 15 minutes: requests 42k, 5xx 0.6% vs baseline 0.3%, p95 310ms vs baseline 285ms, warning pattern doubled for retry exhaustion. Support tickets: none. Business metric checkout_success down 0.8%.
...
```bash
gcloud run deploy post-deploy-judge \
  --source . \
...
2. Runボタンで分析を実行します。
3. 生の運用情報が、どのように `SUCCESS` / `WATCH` / `ROLLBACK` の判断へ変換されるか説明します。
...
- [操作マニュアル](./docs/manual.html): Post Deploy Judge の仮説、操作手順、判断の読み方、デモ確認項目をHTML図解で確認できます。
