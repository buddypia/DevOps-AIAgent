# Two Minute Triage Terraform Deploy Guide
...
![Two Minute Triage architecture](./architecture.svg)
...
```bash
cd outputs/02-two-minute-triage/infra/terraform
terraform init
terraform apply \
...
```bash
printf "%s" "$GEMINI_API_KEY" | gcloud secrets create two-minute-triage-gemini-api-key \
...
terraform apply \
  -var project_id="$GOOGLE_CLOUD_PROJECT" \
  -var existing_gemini_api_secret_id="two-minute-triage-gemini-api-key" \
...
Open `$SERVICE_URL`, run the sample scenario, and confirm a STABILIZE / INVESTIGATE / ESCALATE decision is rendered.
