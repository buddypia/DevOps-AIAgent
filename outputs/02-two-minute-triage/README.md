...
Finds the three things worth checking in the first two minutes.

## 概要

Two Minute Triage は、DevOps x AI Agent Hackathon 向けの `incident first-response agent` です。Compresses Cloud Logging, Monitoring, and recent PR hints into the smallest useful initial incident plan.

MVPでは、スコープを絞って次の体験を作ります。A triage console that ranks evidence, names the likely first owner, and produces a two-minute action script.
...
- 判断ラベル: `STABILIZE` / `INVESTIGATE` / `ESCALATE`
...
Two Minute Triage は、その中でも次の運用判断にフォーカスします。
...
`incident first-response agent` の仕事は影響範囲が大きく、ひとつの誤ったデプロイ判断、ロールバック判断、初動対応、確認漏れが、数時間の調査やユーザー信頼の低下につながります。チームには、次のような軽量な支援役が必要です。

- 雑多な運用情報を素早く読む
- 判断を `STABILIZE` / `INVESTIGATE` / `ESCALATE` のように明確化する
- 判断の根拠を提示する
...
- 人間の担当者は、曖昧な要約ではなく `STABILIZE` / `INVESTIGATE` / `ESCALATE` の具体的な判断を受け取れます。
...
エージェントの人格は `incident first-response agent` です。プロンプトは、プロジェクトの目的と次のフォーカス領域に制約しています。
...
- `decision`: `STABILIZE` / `INVESTIGATE` / `ESCALATE` のいずれか
...
- Target: `checkout-api incident 2026-05-13T09:42:00+09:00`
- Context: PagerDuty fired for checkout-api. Engineers have two minutes before customer support joins the bridge.
- Signals: Cloud Logging: 5xx jumped from 0.2% to 4.9% after revision checkout-api-00153. Monitoring: p99 latency 3.8s, CPU 42%, DB connections normal. Recent PRs: retry policy merged 17 minutes ago, feature flag checkout_retry_v2 enabled for 25%.
...
```bash
gcloud run deploy two-minute-triage \
  --source . \
...
2. Runボタンで分析を実行します。
3. 生の運用情報が、どのように `STABILIZE` / `INVESTIGATE` / `ESCALATE` の判断へ変換されるか説明します。
...
- [操作マニュアル](./docs/manual.html): Two Minute Triage の仮説、操作手順、判断の読み方、デモ確認項目をHTML図解で確認できます。
