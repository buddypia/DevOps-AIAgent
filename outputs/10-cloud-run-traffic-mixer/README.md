# Cloud Run Traffic Mixer

Plans staged Cloud Run revision traffic shifts from live signals.

## 概要

Cloud Run Traffic Mixer は、DevOps x AI Agent Hackathon 向けの `traffic split planning agent` です。Uses revision evidence and operational signals to recommend the next Cloud Run traffic allocation.

MVPでは、スコープを絞って次の体験を作ります。A traffic mixer that proposes 10/25/50/100 percent stages, waits, and rollback conditions.
...
- 判断ラベル: `SHIFT UP` / `HOLD SPLIT` / `SHIFT DOWN`
...
Cloud Run Traffic Mixer は、その中でも次の運用判断にフォーカスします。
...
`traffic split planning agent` の仕事は影響範囲が大きく、ひとつの誤ったデプロイ判断、ロールバック判断、初動対応、確認漏れが、数時間の調査やユーザー信頼の低下につながります。チームには、次のような軽量な支援役が必要です。

- 雑多な運用情報を素早く読む
- 判断を `SHIFT UP` / `HOLD SPLIT` / `SHIFT DOWN` のように明確化する
- 判断の根拠を提示する
...
- 人間の担当者は、曖昧な要約ではなく `SHIFT UP` / `HOLD SPLIT` / `SHIFT DOWN` の具体的な判断を受け取れます。
...
エージェントの人格は `traffic split planning agent` です。プロンプトは、プロジェクトの目的と次のフォーカス領域に制約しています。
...
- `decision`: `SHIFT UP` / `HOLD SPLIT` / `SHIFT DOWN` のいずれか
...
- Target: `checkout-api traffic split`
- Context: New revision receives 25% traffic. Team wants an autonomous recommendation for the next split.
- Signals: Revision stable at 25% for 20 minutes. New revision 5xx 0.42%, old 0.31%. p95 +20ms. Business conversion unchanged. New log pattern appears only when feature flag retry_v2 is on. Rollback command tested.
...
```bash
gcloud run deploy cloud-run-traffic-mixer \
  --source . \
...
2. Runボタンで分析を実行します。
3. 生の運用情報が、どのように `SHIFT UP` / `HOLD SPLIT` / `SHIFT DOWN` の判断へ変換されるか説明します。
...
- [操作マニュアル](./docs/manual.html): Cloud Run Traffic Mixer の仮説、操作手順、判断の読み方、デモ確認項目をHTML図解で確認できます。
