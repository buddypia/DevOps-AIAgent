# Cloud Run Traffic Mixer Terraform Deploy Guide
...
![Cloud Run Traffic Mixer architecture](./architecture.svg)
...
```bash
cd outputs/10-cloud-run-traffic-mixer/infra/terraform
terraform init
terraform apply \
...
```bash
printf "%s" "$GEMINI_API_KEY" | gcloud secrets create cloud-run-traffic-mixer-gemini-api-key \
...
terraform apply \
  -var project_id="$GOOGLE_CLOUD_PROJECT" \
  -var existing_gemini_api_secret_id="cloud-run-traffic-mixer-gemini-api-key" \
...
Open `$SERVICE_URL`, run the sample scenario, and confirm a SHIFT UP / HOLD SPLIT / SHIFT DOWN decision is rendered.
