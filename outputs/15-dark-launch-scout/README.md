...
Reads hidden-feature signals before public launch.

## 概要

Dark Launch Scout は、DevOps x AI Agent Hackathon 向けの `dark launch readiness scout` です。Analyzes internal-only or flag-hidden behavior to judge whether a dark launch is ready for exposure.

MVPでは、スコープを絞って次の体験を作ります。A scout dashboard that scores hidden feature health and names what must be watched before launch.
...
- 判断ラベル: `LAUNCH` / `KEEP DARK` / `DISABLE`
...
Dark Launch Scout は、その中でも次の運用判断にフォーカスします。
...
`dark launch readiness scout` の仕事は影響範囲が大きく、ひとつの誤ったデプロイ判断、ロールバック判断、初動対応、確認漏れが、数時間の調査やユーザー信頼の低下につながります。チームには、次のような軽量な支援役が必要です。

- 雑多な運用情報を素早く読む
- 判断を `LAUNCH` / `KEEP DARK` / `DISABLE` のように明確化する
- 判断の根拠を提示する
...
- 人間の担当者は、曖昧な要約ではなく `LAUNCH` / `KEEP DARK` / `DISABLE` の具体的な判断を受け取れます。
...
エージェントの人格は `dark launch readiness scout` です。プロンプトは、プロジェクトの目的と次のフォーカス領域に制約しています。
...
- `decision`: `LAUNCH` / `KEEP DARK` / `DISABLE` のいずれか
...
- Target: `feature flag invoice_ai_assist`
- Context: New AI invoice assistant is dark-launched to employees only. Product wants to open beta.
- Signals: Internal traffic 840 sessions. Error rate 1.1%, p95 620ms, Gemini timeout 0.4%. Manual feedback: useful but sometimes slow. No external users. Logging lacks request category field. Rollback is flag disable.
...
```bash
gcloud run deploy dark-launch-scout \
  --source . \
...
2. Runボタンで分析を実行します。
3. 生の運用情報が、どのように `LAUNCH` / `KEEP DARK` / `DISABLE` の判断へ変換されるか説明します。
...
- [操作マニュアル](./docs/manual.html): Dark Launch Scout の仮説、操作手順、判断の読み方、デモ確認項目をHTML図解で確認できます。
