# Dark Launch Scout Terraform Deploy Guide
...
![Dark Launch Scout architecture](./architecture.svg)
...
```bash
cd outputs/15-dark-launch-scout/infra/terraform
terraform init
terraform apply \
...
```bash
printf "%s" "$GEMINI_API_KEY" | gcloud secrets create dark-launch-scout-gemini-api-key \
...
terraform apply \
  -var project_id="$GOOGLE_CLOUD_PROJECT" \
  -var existing_gemini_api_secret_id="dark-launch-scout-gemini-api-key" \
...
Open `$SERVICE_URL`, run the sample scenario, and confirm a LAUNCH / KEEP DARK / DISABLE decision is rendered.
