# Model Rollback Agent

Decides when a model or prompt change should be rolled back.

## 概要

Model Rollback Agent は、DevOps x AI Agent Hackathon 向けの `AI model rollback judge` です。Compares model evaluation, production feedback, and logs to judge rollback for AI features.

MVPでは、スコープを絞って次の体験を作ります。A model operations board that recommends keep, canary, or rollback with evidence.
...
- 判断ラベル: `KEEP MODEL` / `CANARY` / `ROLLBACK MODEL`
...
Model Rollback Agent は、その中でも次の運用判断にフォーカスします。
...
`AI model rollback judge` の仕事は影響範囲が大きく、ひとつの誤ったデプロイ判断、ロールバック判断、初動対応、確認漏れが、数時間の調査やユーザー信頼の低下につながります。チームには、次のような軽量な支援役が必要です。

- 雑多な運用情報を素早く読む
- 判断を `KEEP MODEL` / `CANARY` / `ROLLBACK MODEL` のように明確化する
- 判断の根拠を提示する
...
- 人間の担当者は、曖昧な要約ではなく `KEEP MODEL` / `CANARY` / `ROLLBACK MODEL` の具体的な判断を受け取れます。
...
エージェントの人格は `AI model rollback judge` です。プロンプトは、プロジェクトの目的と次のフォーカス領域に制約しています。
...
- `decision`: `KEEP MODEL` / `CANARY` / `ROLLBACK MODEL` のいずれか
...
- Target: `support-agent model update`
- Context: Support agent moved from previous model to Gemini Flash 3.1 variant. Team needs rollback decision after one hour.
- Signals: Eval pass rate 93% -> 90%, tool-call success 96% -> 91%, Japanese answers improved, refund hallucination reports increased from 1 to 7. Latency down 24%. Human override rate up 3.5 points.
...
```bash
gcloud run deploy model-rollback-agent \
  --source . \
...
2. Runボタンで分析を実行します。
3. 生の運用情報が、どのように `KEEP MODEL` / `CANARY` / `ROLLBACK MODEL` の判断へ変換されるか説明します。
...
- [操作マニュアル](./docs/manual.html): Model Rollback Agent の仮説、操作手順、判断の読み方、デモ確認項目をHTML図解で確認できます。
