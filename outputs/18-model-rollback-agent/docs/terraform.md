# Model Rollback Agent Terraform Deploy Guide
...
![Model Rollback Agent architecture](./architecture.svg)
...
```bash
cd outputs/18-model-rollback-agent/infra/terraform
terraform init
terraform apply \
...
```bash
printf "%s" "$GEMINI_API_KEY" | gcloud secrets create model-rollback-agent-gemini-api-key \
...
terraform apply \
  -var project_id="$GOOGLE_CLOUD_PROJECT" \
  -var existing_gemini_api_secret_id="model-rollback-agent-gemini-api-key" \
...
Open `$SERVICE_URL`, run the sample scenario, and confirm a KEEP MODEL / CANARY / ROLLBACK MODEL decision is rendered.
