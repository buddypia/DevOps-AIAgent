...
Canary Diff Judge は、DevOps x AI Agent Hackathon 向けの `canary promotion judge` です。Reads old/new Cloud Run revision data, log deltas, latency changes, and error rates to judge canary promotion.

MVPでは、スコープを絞って次の体験を作ります。A dashboard that receives canary evidence and returns a promotion verdict with traffic-shift instructions.
...
Canary Diff Judge は、その中でも次の運用判断にフォーカスします。
...
`canary promotion judge` の仕事は影響範囲が大きく、ひとつの誤ったデプロイ判断、ロールバック判断、初動対応、確認漏れが、数時間の調査やユーザー信頼の低下につながります。チームには、次のような軽量な支援役が必要です。

- 雑多な運用情報を素早く読む
- 判断を `PROMOTE` / `HOLD` / `ROLLBACK` のように明確化する
- 判断の根拠を提示する
...
- 人間の担当者は、曖昧な要約ではなく `PROMOTE` / `HOLD` / `ROLLBACK` の具体的な判断を受け取れます。
...
エージェントの人格は `canary promotion judge` です。プロンプトは、プロジェクトの目的と次のフォーカス領域に制約しています。
...
- `decision`: `PROMOTE` / `HOLD` / `ROLLBACK` のいずれか
...
- Target: `cloud-run://checkout-api/revisions/00152..00153`
- Context: New revision is serving 10% traffic. Goal is to decide whether to move to 50% or roll back.
- Signals: Old revision: 0.3% 5xx, p95 280ms, 2 warning patterns. New revision: 0.7% 5xx, p95 318ms, new warning 'payment retry exhausted' 14 times, no customer tickets.
...
```bash
gcloud run deploy canary-diff-judge \
  --source . \
...
2. Runボタンで分析を実行します。
3. 生の運用情報が、どのように `PROMOTE` / `HOLD` / `ROLLBACK` の判断へ変換されるか説明します。
...
- [操作マニュアル](./docs/manual.html): Canary Diff Judge の仮説、操作手順、判断の読み方、デモ確認項目をHTML図解で確認できます。
