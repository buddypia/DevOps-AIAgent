# Incident Commander Karaoke Terraform Deploy Guide
...
![Incident Commander Karaoke architecture](./architecture.svg)
...
```bash
cd outputs/20-incident-commander-karaoke/infra/terraform
terraform init
terraform apply \
...
```bash
printf "%s" "$GEMINI_API_KEY" | gcloud secrets create incident-commander-karaoke-gemini-api-key \
...
terraform apply \
  -var project_id="$GOOGLE_CLOUD_PROJECT" \
  -var existing_gemini_api_secret_id="incident-commander-karaoke-gemini-api-key" \
...
Open `$SERVICE_URL`, run the sample scenario, and confirm a GOOD COMMAND / COACH / RETRY COMMAND decision is rendered.
