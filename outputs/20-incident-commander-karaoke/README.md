# Incident Commander Karaoke

Scores spoken incident commands for clarity and operational fit.

## 概要

Incident Commander Karaoke は、DevOps x AI Agent Hackathon 向けの `incident command rehearsal coach` です。Evaluates incident commander instructions from transcript-like input and returns coaching feedback.

MVPでは、スコープを絞って次の体験を作ります。A rehearsal console that scores a command transcript, identifies missing facts, and drafts a better command.
...
- 判断ラベル: `GOOD COMMAND` / `COACH` / `RETRY COMMAND`
...
Incident Commander Karaoke は、その中でも次の運用判断にフォーカスします。
...
`incident command rehearsal coach` の仕事は影響範囲が大きく、ひとつの誤ったデプロイ判断、ロールバック判断、初動対応、確認漏れが、数時間の調査やユーザー信頼の低下につながります。チームには、次のような軽量な支援役が必要です。

- 雑多な運用情報を素早く読む
- 判断を `GOOD COMMAND` / `COACH` / `RETRY COMMAND` のように明確化する
- 判断の根拠を提示する
...
- 人間の担当者は、曖昧な要約ではなく `GOOD COMMAND` / `COACH` / `RETRY COMMAND` の具体的な判断を受け取れます。
...
エージェントの人格は `incident command rehearsal coach` です。プロンプトは、プロジェクトの目的と次のフォーカス領域に制約しています。
...
- `decision`: `GOOD COMMAND` / `COACH` / `RETRY COMMAND` のいずれか
...
- Target: `incident commander transcript`
- Context: A practice incident bridge is running. The commander reads a short instruction and wants scoring.
- Signals: Transcript: 'Everyone look at logs and tell me if it is bad. Maybe roll back if needed. Someone update support.' Missing: named owners, time box, user impact, rollback criteria, next update time. Situation: checkout 5xx at 4.8%.
...
```bash
gcloud run deploy incident-commander-karaoke \
  --source . \
...
2. Runボタンで分析を実行します。
3. 生の運用情報が、どのように `GOOD COMMAND` / `COACH` / `RETRY COMMAND` の判断へ変換されるか説明します。
...
- [操作マニュアル](./docs/manual.html): Incident Commander Karaoke の仮説、操作手順、判断の読み方、デモ確認項目をHTML図解で確認できます。
