# Runbook Decay Detector

Finds stale commands, URLs, env vars, and ownership in runbooks.

## 概要

Runbook Decay Detector は、DevOps x AI Agent Hackathon 向けの `runbook freshness agent` です。Compares runbook text with current repo and deployment evidence to generate a repair plan.

MVPでは、スコープを絞って次の体験を作ります。A runbook review app that marks decay points and drafts a documentation PR comment.
...
Runbook Decay Detector は、その中でも次の運用判断にフォーカスします。
...
`runbook freshness agent` の仕事は影響範囲が大きく、ひとつの誤ったデプロイ判断、ロールバック判断、初動対応、確認漏れが、数時間の調査やユーザー信頼の低下につながります。チームには、次のような軽量な支援役が必要です。

- 雑多な運用情報を素早く読む
- 判断を `FRESH` / `PATCH DOCS` / `STALE` のように明確化する
- 判断の根拠を提示する
...
- 人間の担当者は、曖昧な要約ではなく `FRESH` / `PATCH DOCS` / `STALE` の具体的な判断を受け取れます。
...
エージェントの人格は `runbook freshness agent` です。プロンプトは、プロジェクトの目的と次のフォーカス領域に制約しています。
...
- `decision`: `FRESH` / `PATCH DOCS` / `STALE` のいずれか
...
- Target: `docs/runbooks/payment-timeout.md`
- Context: Runbook has not been edited for four months. Service migrated from payment-api to checkout-api.
- Signals: Runbook command references gcloud run services update payment-api. Current service is checkout-api. Env var PAYMENT_TIMEOUT_MS renamed CHECKOUT_PAYMENT_TIMEOUT_MS. Pager rotation owner listed as old Slack channel #payments-ops.
...
```bash
gcloud run deploy runbook-decay-detector \
  --source . \
...
2. Runボタンで分析を実行します。
3. 生の運用情報が、どのように `FRESH` / `PATCH DOCS` / `STALE` の判断へ変換されるか説明します。
...
- [操作マニュアル](./docs/manual.html): Runbook Decay Detector の仮説、操作手順、判断の読み方、デモ確認項目をHTML図解で確認できます。
