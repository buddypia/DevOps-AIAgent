# Product Strategy: Runbook Decay Detector
...
- Urgent pain: runbook freshness agent decisions require scattered evidence and are still made by whoever happens to be online.

## Job To Be Done

When command freshness, env var drift, owner drift create ambiguity, the user wants to turn raw operational evidence into a decision, owner, verification command, and shareable comment before the window closes.

## Wedge Hypothesis

If Runbook Decay Detector can make one high-pressure command freshness decision faster and easier to explain, teams will first use it as a review/incident assistant, then ask for deeper integrations with GitHub, Cloud Logging, Cloud Monitoring, Slack, and audit storage.
