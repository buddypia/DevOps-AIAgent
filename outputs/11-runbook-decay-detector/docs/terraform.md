# Runbook Decay Detector Terraform Deploy Guide
...
![Runbook Decay Detector architecture](./architecture.svg)
...
```bash
cd outputs/11-runbook-decay-detector/infra/terraform
terraform init
terraform apply \
...
```bash
printf "%s" "$GEMINI_API_KEY" | gcloud secrets create runbook-decay-detector-gemini-api-key \
...
terraform apply \
  -var project_id="$GOOGLE_CLOUD_PROJECT" \
  -var existing_gemini_api_secret_id="runbook-decay-detector-gemini-api-key" \
...
Open `$SERVICE_URL`, run the sample scenario, and confirm a FRESH / PATCH DOCS / STALE decision is rendered.
