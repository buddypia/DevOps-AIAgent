# ShipGuard AI Terraform Deploy Guide
...
![ShipGuard AI architecture](./architecture.svg)
...
```bash
printf "%s" "$GEMINI_API_KEY" | gcloud secrets create shipguard-ai-gemini-api-key \
...
terraform apply \
  -var project_id="$GOOGLE_CLOUD_PROJECT" \
  -var existing_gemini_api_secret_id="shipguard-ai-gemini-api-key" \
...
Open `$SERVICE_URL`, run the sample scenario, and confirm a SHIP / WATCH / BLOCK decision is rendered.
