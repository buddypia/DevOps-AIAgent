# Data Pipeline Sheriff

Judges ETL delay, missing data, and rerun safety.

## 概要

Data Pipeline Sheriff は、DevOps x AI Agent Hackathon 向けの `data pipeline operations agent` です。Monitors ETL logs, lateness, and missing partitions to decide rerun, hold, or escalate.

MVPでは、スコープを絞って次の体験を作ります。A pipeline incident board that recommends data recovery actions and downstream communication.
...
- 判断ラベル: `RERUN` / `HOLD DATA` / `ESCALATE`
...
Data Pipeline Sheriff は、その中でも次の運用判断にフォーカスします。
...
`data pipeline operations agent` の仕事は影響範囲が大きく、ひとつの誤ったデプロイ判断、ロールバック判断、初動対応、確認漏れが、数時間の調査やユーザー信頼の低下につながります。チームには、次のような軽量な支援役が必要です。

- 雑多な運用情報を素早く読む
- 判断を `RERUN` / `HOLD DATA` / `ESCALATE` のように明確化する
- 判断の根拠を提示する
...
- 人間の担当者は、曖昧な要約ではなく `RERUN` / `HOLD DATA` / `ESCALATE` の具体的な判断を受け取れます。
...
エージェントの人格は `data pipeline operations agent` です。プロンプトは、プロジェクトの目的と次のフォーカス領域に制約しています。
...
- `decision`: `RERUN` / `HOLD DATA` / `ESCALATE` のいずれか
...
- Target: `daily_revenue_pipeline 2026-05-13`
- Context: The daily revenue BigQuery table is late. Finance dashboard refresh is due in 35 minutes.
- Signals: ETL logs: extract complete, transform failed on null currency_code, partition 2026-05-12 missing 18% rows. Upstream source emitted schema warning. Last successful run 24h ago. Rerun cost estimate low. Dashboard SLA 11:00 JST.
...
```bash
gcloud run deploy data-pipeline-sheriff \
  --source . \
...
2. Runボタンで分析を実行します。
3. 生の運用情報が、どのように `RERUN` / `HOLD DATA` / `ESCALATE` の判断へ変換されるか説明します。
...
- [操作マニュアル](./docs/manual.html): Data Pipeline Sheriff の仮説、操作手順、判断の読み方、デモ確認項目をHTML図解で確認できます。
