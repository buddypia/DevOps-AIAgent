# Data Pipeline Sheriff Terraform Deploy Guide
...
![Data Pipeline Sheriff architecture](./architecture.svg)
...
```bash
cd outputs/12-data-pipeline-sheriff/infra/terraform
terraform init
terraform apply \
...
```bash
printf "%s" "$GEMINI_API_KEY" | gcloud secrets create data-pipeline-sheriff-gemini-api-key \
...
terraform apply \
  -var project_id="$GOOGLE_CLOUD_PROJECT" \
  -var existing_gemini_api_secret_id="data-pipeline-sheriff-gemini-api-key" \
...
Open `$SERVICE_URL`, run the sample scenario, and confirm a RERUN / HOLD DATA / ESCALATE decision is rendered.
