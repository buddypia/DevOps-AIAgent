# Deployment Black Box Recorder Terraform Deploy Guide
...
![Deployment Black Box Recorder architecture](./architecture.svg)
...
```bash
cd outputs/14-deployment-black-box-recorder/infra/terraform
terraform init
terraform apply \
...
```bash
printf "%s" "$GEMINI_API_KEY" | gcloud secrets create deployment-black-box-recorder-gemini-api-key \
...
terraform apply \
  -var project_id="$GOOGLE_CLOUD_PROJECT" \
  -var existing_gemini_api_secret_id="deployment-black-box-recorder-gemini-api-key" \
...
Open `$SERVICE_URL`, run the sample scenario, and confirm a RECORD / ENRICH RECORD / MISSING EVIDENCE decision is rendered.
