# Deployment Black Box Recorder

Captures the facts that matter at deploy time.

## 概要

Deployment Black Box Recorder は、DevOps x AI Agent Hackathon 向けの `deployment evidence recorder` です。Builds a searchable record of PRs, CI, config, AI judgment, and logs for later incident learning.

MVPでは、スコープを絞って次の体験を作ります。A deploy recorder that summarizes the deployment envelope and highlights evidence worth preserving.
...
- 判断ラベル: `RECORD` / `ENRICH RECORD` / `MISSING EVIDENCE`
...
Deployment Black Box Recorder は、その中でも次の運用判断にフォーカスします。
...
`deployment evidence recorder` の仕事は影響範囲が大きく、ひとつの誤ったデプロイ判断、ロールバック判断、初動対応、確認漏れが、数時間の調査やユーザー信頼の低下につながります。チームには、次のような軽量な支援役が必要です。

- 雑多な運用情報を素早く読む
- 判断を `RECORD` / `ENRICH RECORD` / `MISSING EVIDENCE` のように明確化する
- 判断の根拠を提示する
...
- 人間の担当者は、曖昧な要約ではなく `RECORD` / `ENRICH RECORD` / `MISSING EVIDENCE` の具体的な判断を受け取れます。
...
エージェントの人格は `deployment evidence recorder` です。プロンプトは、プロジェクトの目的と次のフォーカス領域に制約しています。
...
- `decision`: `RECORD` / `ENRICH RECORD` / `MISSING EVIDENCE` のいずれか
...
- Target: `deploy record checkout-api 00153`
- Context: Team wants every deployment to leave a forensic capsule for postmortem and release review.
- Signals: Captured: PR 184, CI run 7721, image digest sha256:abc, env diff with RETRY_LIMIT 2->4, traffic split 10->100, release owner, AI gate verdict WATCH. Missing: final 15-minute log snapshot and feature flag state.
...
```bash
gcloud run deploy deployment-black-box-recorder \
  --source . \
...
2. Runボタンで分析を実行します。
3. 生の運用情報が、どのように `RECORD` / `ENRICH RECORD` / `MISSING EVIDENCE` の判断へ変換されるか説明します。
...
- [操作マニュアル](./docs/manual.html): Deployment Black Box Recorder の仮説、操作手順、判断の読み方、デモ確認項目をHTML図解で確認できます。
