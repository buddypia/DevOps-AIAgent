# Deploy Rehearsal Agent

Dry-runs deployment instructions before they become an outage.

## 概要

Deploy Rehearsal Agent は、DevOps x AI Agent Hackathon 向けの `pre-production rehearsal director` です。Reads README, Actions, environment variables, and release notes to catch deploy holes before production.

MVPでは、スコープを絞って次の体験を作ります。A rehearsal checker that produces missing prerequisites, command order, and stop conditions.
...
- 判断ラベル: `READY` / `REHEARSE` / `NOT READY`
...
Deploy Rehearsal Agent は、その中でも次の運用判断にフォーカスします。
...
`pre-production rehearsal director` の仕事は影響範囲が大きく、ひとつの誤ったデプロイ判断、ロールバック判断、初動対応、確認漏れが、数時間の調査やユーザー信頼の低下につながります。チームには、次のような軽量な支援役が必要です。

- 雑多な運用情報を素早く読む
- 判断を `READY` / `REHEARSE` / `NOT READY` のように明確化する
- 判断の根拠を提示する
...
- 人間の担当者は、曖昧な要約ではなく `READY` / `REHEARSE` / `NOT READY` の具体的な判断を受け取れます。
...
エージェントの人格は `pre-production rehearsal director` です。プロンプトは、プロジェクトの目的と次のフォーカス領域に制約しています。
...
- `decision`: `READY` / `REHEARSE` / `NOT READY` のいずれか
...
- Context: Team wants to deploy a Cloud Run service from source after merging feature flags and DB index migration.
- Signals: README says set STRIPE_WEBHOOK_SECRET but .env.example lacks it. GitHub Actions deploy job uses node 20 while local package expects node 22. Cloud Run min instances set to zero. Rollback command exists but migration rollback SQL is missing.
...
```bash
gcloud run deploy deploy-rehearsal-agent \
  --source . \
...
2. Runボタンで分析を実行します。
3. 生の運用情報が、どのように `READY` / `REHEARSE` / `NOT READY` の判断へ変換されるか説明します。
...
- [操作マニュアル](./docs/manual.html): Deploy Rehearsal Agent の仮説、操作手順、判断の読み方、デモ確認項目をHTML図解で確認できます。
