# Deploy Rehearsal Agent Terraform Deploy Guide
...
![Deploy Rehearsal Agent architecture](./architecture.svg)
...
```bash
cd outputs/06-deploy-rehearsal-agent/infra/terraform
terraform init
terraform apply \
...
```bash
printf "%s" "$GEMINI_API_KEY" | gcloud secrets create deploy-rehearsal-agent-gemini-api-key \
...
terraform apply \
  -var project_id="$GOOGLE_CLOUD_PROJECT" \
  -var existing_gemini_api_secret_id="deploy-rehearsal-agent-gemini-api-key" \
...
Open `$SERVICE_URL`, run the sample scenario, and confirm a READY / REHEARSE / NOT READY decision is rendered.
