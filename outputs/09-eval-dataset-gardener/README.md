# Eval Dataset Gardener

Turns failures and low-rated responses into eval cases.

## 概要

Eval Dataset Gardener は、DevOps x AI Agent Hackathon 向けの `AI eval maintenance agent` です。Harvests failed AI responses and operational feedback into structured evaluation cases for regression gates.

MVPでは、スコープを絞って次の体験を作ります。A dataset workbench that suggests eval cases, expected behavior, and a regression run plan.
...
- 判断ラベル: `ADD CASES` / `CURATE` / `BLOCK MODEL CHANGE`
...
Eval Dataset Gardener は、その中でも次の運用判断にフォーカスします。
...
`AI eval maintenance agent` の仕事は影響範囲が大きく、ひとつの誤ったデプロイ判断、ロールバック判断、初動対応、確認漏れが、数時間の調査やユーザー信頼の低下につながります。チームには、次のような軽量な支援役が必要です。

- 雑多な運用情報を素早く読む
- 判断を `ADD CASES` / `CURATE` / `BLOCK MODEL CHANGE` のように明確化する
- 判断の根拠を提示する
...
- 人間の担当者は、曖昧な要約ではなく `ADD CASES` / `CURATE` / `BLOCK MODEL CHANGE` の具体的な判断を受け取れます。
...
エージェントの人格は `AI eval maintenance agent` です。プロンプトは、プロジェクトの目的と次のフォーカス領域に制約しています。
...
- `decision`: `ADD CASES` / `CURATE` / `BLOCK MODEL CHANGE` のいずれか
...
- Context: AI support agent had low-rated answers after a prompt update. Team needs new regression cases before the next release.
- Signals: Failed responses: refund policy hallucinated for enterprise plan, ignored Japanese locale, tool timeout turned into confident answer. User ratings: 12 thumbs down. Current eval suite lacks locale and tool-failure cases.
...
```bash
gcloud run deploy eval-dataset-gardener \
  --source . \
...
2. Runボタンで分析を実行します。
3. 生の運用情報が、どのように `ADD CASES` / `CURATE` / `BLOCK MODEL CHANGE` の判断へ変換されるか説明します。
...
- [操作マニュアル](./docs/manual.html): Eval Dataset Gardener の仮説、操作手順、判断の読み方、デモ確認項目をHTML図解で確認できます。
