# Eval Dataset Gardener Terraform Deploy Guide
...
![Eval Dataset Gardener architecture](./architecture.svg)
...
```bash
cd outputs/09-eval-dataset-gardener/infra/terraform
terraform init
terraform apply \
...
```bash
printf "%s" "$GEMINI_API_KEY" | gcloud secrets create eval-dataset-gardener-gemini-api-key \
...
terraform apply \
  -var project_id="$GOOGLE_CLOUD_PROJECT" \
  -var existing_gemini_api_secret_id="eval-dataset-gardener-gemini-api-key" \
...
Open `$SERVICE_URL`, run the sample scenario, and confirm a ADD CASES / CURATE / BLOCK MODEL CHANGE decision is rendered.
