...
Checks whether recovery is real or just temporarily quiet.

## 概要

Recovery Confidence Meter は、DevOps x AI Agent Hackathon 向けの `recovery verification agent` です。Compares recovery-period logs and metrics against healthy baselines to determine confidence.

MVPでは、スコープを絞って次の体験を作ります。A recovery meter with confidence scoring, residual risk, and evidence to close or continue the incident.
...
- 判断ラベル: `CLOSE` / `VERIFY` / `KEEP INCIDENT OPEN`
...
Recovery Confidence Meter は、その中でも次の運用判断にフォーカスします。
...
`recovery verification agent` の仕事は影響範囲が大きく、ひとつの誤ったデプロイ判断、ロールバック判断、初動対応、確認漏れが、数時間の調査やユーザー信頼の低下につながります。チームには、次のような軽量な支援役が必要です。

- 雑多な運用情報を素早く読む
- 判断を `CLOSE` / `VERIFY` / `KEEP INCIDENT OPEN` のように明確化する
- 判断の根拠を提示する
...
- 人間の担当者は、曖昧な要約ではなく `CLOSE` / `VERIFY` / `KEEP INCIDENT OPEN` の具体的な判断を受け取れます。
...
エージェントの人格は `recovery verification agent` です。プロンプトは、プロジェクトの目的と次のフォーカス領域に制約しています。
...
- `decision`: `CLOSE` / `VERIFY` / `KEEP INCIDENT OPEN` のいずれか
...
- Target: `incident INC-1042 recovery review`
- Context: A payment timeout incident was mitigated by rolling back one Cloud Run revision. Team wants to close the incident.
- Signals: After rollback: 5xx 0.25%, p95 290ms, retry warnings 5/min down from 180/min. Checkout success back to 98.8% baseline. One customer report arrived 6 minutes after rollback. DB latency normal.
...
```bash
gcloud run deploy recovery-confidence-meter \
  --source . \
...
2. Runボタンで分析を実行します。
3. 生の運用情報が、どのように `CLOSE` / `VERIFY` / `KEEP INCIDENT OPEN` の判断へ変換されるか説明します。
...
- [操作マニュアル](./docs/manual.html): Recovery Confidence Meter の仮説、操作手順、判断の読み方、デモ確認項目をHTML図解で確認できます。
