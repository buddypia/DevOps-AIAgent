# Recovery Confidence Meter Terraform Deploy Guide
...
![Recovery Confidence Meter architecture](./architecture.svg)
...
```bash
cd outputs/08-recovery-confidence-meter/infra/terraform
terraform init
terraform apply \
...
```bash
printf "%s" "$GEMINI_API_KEY" | gcloud secrets create recovery-confidence-meter-gemini-api-key \
...
terraform apply \
  -var project_id="$GOOGLE_CLOUD_PROJECT" \
  -var existing_gemini_api_secret_id="recovery-confidence-meter-gemini-api-key" \
...
Open `$SERVICE_URL`, run the sample scenario, and confirm a CLOSE / VERIFY / KEEP INCIDENT OPEN decision is rendered.
