...
ProtoPediaには、公開GitHubリポジトリURL、Cloud Run URL、作品URLを提出します。GitHubは <https://github.com/buddypia/DevOps-AIAgent>、Cloud Runは <https://a2a-agent-marketplace-xhdqpudx6a-an.a.run.app>。公開CIは <https://github.com/buddypia/DevOps-AIAgent/actions/workflows/ci.yml>。タグは `findy_hackathon`。提出本文はSubmission Publisherのpaste fieldsを貼り、動画ではPitch Directorの「Judge Proof → Finalist Simulator → Submission Publisher → Marketplace → Winning Strategy → Mission Control → Ops Drill」の30秒リールを録画します。質疑ではJudge Drillのhardest questionとFinalist Simulatorのtop concern/evidence linksを使います。
