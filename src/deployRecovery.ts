...
export function buildDeployRecoveryPlan(input: {
  ...
  const commands: DeployRecoveryCommand[] = [
    ...
    },
    {
      ...
      command: `curl -s -X POST ${targetBaseUrl}/a2a -H 'Content-Type: application/json' --data '{"method":"message/send","params":{"text":"A2A Cloud Run Gemini DevOps"}}' | jq '.result.artifacts[0].parts[0].data | {deployRecoveryEndpoint, deployRecoveryPageEndpoint, prizeStrategyPageEndpoint, publisherPageEndpoint}'`,
      why: "A2A artifactがDeploy Recovery API/GET証拠ページ、Prize Strategy GET証拠ページ、Publisher GET証拠ページを公開しているかを確認します。",
      copyGroup: "verify",
      blocking: false
    }
  ];
  ...
  const judgeScript = [
    ...
    `After deploy: verify Agent Card count, /win-autopilot, /winner-sufficiency, /observability-oracle, /api/mvp-readiness, /api/autonomy-snapshot, /api/recording-script, /prize-strategy, /publisher, /api/pilot-value, /deploy-recovery, /api/deploy-recovery, and A2A autonomySnapshot/recordingScript/prizeStrategy/publisher/pilotValue/deployRecovery endpoints.`
  ];
