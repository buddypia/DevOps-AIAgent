...
export function buildReleaseDriftGuard(input: {
  ...
  return {
    ...
    runbook: [
      ...
      "gh workflow run verify-public-proof.yml --ref main -f target_url=https://a2a-agent-marketplace-xhdqpudx6a-an.a.run.app",
      "gcloud auth login",
      ...
      `curl -s ${targetBaseUrl}/.well-known/agent-card.json | jq '.skills[] | select(.id=="judge.command" or .id=="judge.rehearsal" or .id=="win.gap.radar" or .id=="winner.packet" or .id=="winner.sufficiency" or .id=="win.autopilot" or .id=="judge.objection-arena" or .id=="finalist.simulate" or .id=="competitive.battlecard" or .id=="competitive.snapshot" or .id=="competitive.decision-matrix" or .id=="judge.snapshot" or .id=="judge.first-click" or .id=="mvp.snapshot" or .id=="autonomy.snapshot" or .id=="external.evidence" or .id=="acceptance.matrix" or .id=="recording.script" or .id=="prize.strategy" or .id=="submission.publish" or .id=="submission.dossier" or .id=="submission.launch" or .id=="submission.package" or .id=="pilot.value.snapshot" or .id=="deploy.recover") | {id, tags}'`,
      `homepage_asset=$(curl -s ${targetBaseUrl}/ | sed -n 's|.*src="/assets/\\(index-[^"]*\\.js\\)".*|\\1|p' | head -1); curl -s ${targetBaseUrl}/assets/$homepage_asset | rg 'Buyer Pilot Contract Builder|Buyer-facing artifact|Open buyer brief|Start workflow intake|First action route lock'`,
      `curl -s ${targetBaseUrl}/sample/buyer-outcome-brief | rg 'Buyer Outcome Brief'`,
      `curl -s ${targetBaseUrl}/api/sample/buyer-outcome-brief | jq '{decision, liveProof: (.metrics[] | select(.id=="live-proof") | .value)}'`,
      `curl -s ${targetBaseUrl}/sample/pilot-run-receipt | rg 'Pilot Receipt|Pilot Run Receipt'`,
      `curl -s ${targetBaseUrl}/sample/work-order-brief | rg 'Buyer Work Order Brief'`,
      `curl -s ${targetBaseUrl}/buyer-proof-monitor | rg 'Proof monitor|Buyer proof monitor'`,
      `curl -s ${targetBaseUrl}/buyer-proof-recovery | rg 'Recovery desk|buyer proof recovery desk'`,
      ...
      `curl -s ${targetBaseUrl}/competitive-decision-matrix | rg 'Competitive Decision Matrix|Head-to-Head Matrix'`,
      `curl -s ${targetBaseUrl}/api/competitive-decision-matrix | jq '{readiness, matrixScore, cells: .summary.cellCount, page: .a2aPayload.endpoints.competitiveDecisionMatrix}'`,
      ...
      `curl -s ${targetBaseUrl}/prize-strategy | rg 'Prize Strategy Proof|Five-Criterion Score Plan'`,
      `curl -s ${targetBaseUrl}/api/prize-strategy | jq '{readiness, prizeScore, criteria: (.criteria | length), page: .a2aPayload.endpoints.prizeStrategyPage}'`,
      `curl -s ${targetBaseUrl}/publisher | rg 'Submission Publisher Proof|ProtoPedia Quality Lock'`,
      `curl -s ${targetBaseUrl}/api/publisher | jq '{readiness, publishScore, quality: .qualityLock.readiness, page: .a2aPayload.endpoints.publisherPage}'`,
      `curl -s ${targetBaseUrl}/dossier | rg 'Submission Dossier Proof|Handoff Packet'`,
      `curl -s ${targetBaseUrl}/api/dossier | jq '{readiness, dossierScore, blocks: (.copyBlocks | length), page: .a2aPayload.endpoints.dossierPage}'`,
      ...
      `curl -s ${targetBaseUrl}/acceptance-matrix | rg 'Acceptance Matrix Proof|Acceptance Rows'`,
      `curl -s ${targetBaseUrl}/api/acceptance-matrix | jq '{verdict, acceptanceScore, rows: (.rows | length)}'`,
    ...
    a2aPayload: {
      ...
      endpoints: {
        ...
        targetCompetitiveDecisionMatrixPage: `${targetBaseUrl}/competitive-decision-matrix`,
        targetCompetitiveDecisionMatrix: `${targetBaseUrl}/api/competitive-decision-matrix`,
        targetRecordingScript: `${targetBaseUrl}/api/recording-script`,
        targetPrizeStrategyPage: `${targetBaseUrl}/prize-strategy`,
        targetPrizeStrategy: `${targetBaseUrl}/api/prize-strategy`,
        targetPublisherPage: `${targetBaseUrl}/publisher`,
        targetPublisher: `${targetBaseUrl}/api/publisher`,
        targetDossierPage: `${targetBaseUrl}/dossier`,
        targetDossier: `${targetBaseUrl}/api/dossier`,
