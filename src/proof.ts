...
export function buildJudgeProof(input: {
  ...
  const proofItems: ProofItem[] = [
    ...
    },
    {
      ...
      evidence:
        "Agent Card exposes market.discover, agent.hire, contract.issue, task.delegate, strategy.audit, market.intel, moat.stress, competitive.battlecard, mvp.audit, acceptance.matrix, judge.brief, judge.command, judge.tour, user.pilot, squad.optimize, evidence.monitor, release.drift, deploy.recover, demo.receipt, autonomy.ledger, mission.run, submission.package, submission.publish, submission.dossier, submission.launch, security.review, impact.case, pilot.economics, demo.runway, win.autopilot, ops.drill, ci.verify, pitch.director, judge.drill, finalist.simulate, and judge.proof.",
      url: links.agentCard
    },
    {
      id: "strategy",
