...
export function buildJudgeProof(input: {
  ...
  const proofItems: ProofItem[] = [
    ...
    },
    {
      ...
      evidence:
        "Agent Card exposes market.discover, agent.hire, contract.issue, task.delegate, strategy.audit, market.intel, moat.stress, competitive.battlecard, competitive.snapshot, judge.snapshot, mvp.audit, mvp.snapshot, acceptance.matrix, judge.brief, judge.command, judge.rehearsal, winner.packet, winner.sufficiency, submission.runway, submission.assets, recording.script, pilot.value.snapshot, prize.strategy, win.gap.radar, judge.tour, user.pilot, squad.optimize, evidence.monitor, observability.oracle, release.drift, deploy.recover, demo.receipt, autonomy.ledger, autonomy.snapshot, mission.run, submission.package, submission.publish, submission.dossier, submission.launch, submission.closeout, security.review, impact.case, pilot.economics, demo.runway, win.autopilot, ops.drill, ci.verify, pitch.director, judge.drill, finalist.simulate, and judge.proof.",
      url: links.agentCard
    },
    {
      id: "strategy",
