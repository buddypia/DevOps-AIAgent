...
describe("judge acceptance matrix", () => {
  ...
  test("does not accept the MVP when the deployed Cloud Run revision is stale", () => {
    ...
    const releaseDrift = buildReleaseDriftGuard({
      ...
      expectedSkillIds: ["evidence.monitor", "demo.receipt", "acceptance.matrix", "release.drift", "pilot.economics", "demo.concierge", "judge.command", "judge.rehearsal", "prize.strategy", "win.gap.radar", "submission.closeout", "deploy.recover", "competitive.battlecard"],
      observedSkillIds: ["evidence.monitor"],
      requiredSkillIds: ["evidence.monitor", "demo.receipt", "acceptance.matrix", "release.drift", "pilot.economics", "demo.concierge", "judge.command", "judge.rehearsal", "prize.strategy", "win.gap.radar", "submission.closeout", "deploy.recover", "competitive.battlecard"],
      probes: [
