...
- Submission Closeoutで、ProtoPedia貼付、ProtoPedia Quality Lock、Publication Policy Lock、構成図、30秒動画、Video Proof Lock、外部URL、最終提出フォームを順番付きの作業台にする
...
React UI
  ...
  -> Demo Concierge persona first-click lanes + Judge Route Lock + First-Run Focus Lock
...
| Time | Shot |
| --- | --- |
| 0-4s | Demo Conciergeで、Judge Route LockとFirst-Run Focus Lockを見せ、機能一覧ではなく証拠順から始める |
...
curl -s -X POST ${PUBLIC_BASE_URL:-http://localhost:8080}/api/release-drift \
  -H 'Content-Type: application/json' \
  --data '{"projectBrief":"A2A Cloud Run Gemini DevOps","selectedAgentIds":["market-broker","gemini-strategist","cloud-run-sre"],"targetUrl":"https://a2a-agent-marketplace-rfyriohodq-an.a.run.app"}'
...
curl -s -X POST ${PUBLIC_BASE_URL:-http://localhost:8080}/api/submission-launch \
  -H 'Content-Type: application/json' \
  --data '{"projectBrief":"A2A Cloud Run Gemini DevOps","selectedAgentIds":["market-broker","gemini-strategist","cloud-run-sre"],"protopediaUrl":"https://protopedia.net/prototype/999999","videoUrl":"https://youtu.be/demo1234567"}'
curl -s -X POST ${PUBLIC_BASE_URL:-http://localhost:8080}/api/submission-closeout \
  -H 'Content-Type: application/json' \
  --data '{"projectBrief":"A2A Cloud Run Gemini DevOps","selectedAgentIds":["market-broker","gemini-strategist","cloud-run-sre"],"protopediaUrl":"https://protopedia.net/prototype/999999","videoUrl":"https://youtu.be/demo1234567"}'
...
- Deployed URL: <https://a2a-agent-marketplace-rfyriohodq-an.a.run.app>
