# 014-deployment-black-box-recorder: Deployment Black Box Recorder
...
| 用語 | 定義 | コード表現 |
|---|---|---|
| Agent | deployment evidence recorderとして判断を行うAI役割 | agent |
| Decision | RECORD / ENRICH RECORD / MISSING EVIDENCEの判断ラベル | decision |
...
| レイヤー | 実体 | 作業 | 条件 |
|---|---|:---:|---|
| Type/Schema | outputs/14-deployment-black-box-recorder/src/project.ts | 🔄 | 実装済み型・Zod契約 |
| Type/Schema | outputs/14-deployment-black-box-recorder/src/agent.ts | 🔄 | 実装済み型・Zod契約 |
| API/Service | outputs/14-deployment-black-box-recorder/src/server.ts | 🔄 | API/外部サービス境界 |
| UI | outputs/14-deployment-black-box-recorder/src/main.ts | 🔄 | UIまたは表示層 |
| Test | outputs/14-deployment-black-box-recorder/src/server.test.ts | ✅ | 回帰・契約テスト |

#### 0.1.1 Traceability Matrix

| FR-ID | 実装ファイル | テスト | 状態 |
|---|---|---|:---:|
| FR-01401 | outputs/14-deployment-black-box-recorder/src/server.ts | outputs/14-deployment-black-box-recorder/src/server.test.ts | 実装済み |
| FR-01402 | outputs/14-deployment-black-box-recorder/src/agent.ts | outputs/14-deployment-black-box-recorder/src/server.test.ts | 実装済み |
| FR-01403 | outputs/14-deployment-black-box-recorder/src/main.ts | outputs/14-deployment-black-box-recorder/src/server.test.ts | 実装済み |
| FR-01404 | outputs/14-deployment-black-box-recorder/src/server.ts | outputs/14-deployment-black-box-recorder/src/server.test.ts | 実装済み |
...
システム役割は「deployment evidence recorder」。入力のevidence capture・timeline anchors・forensic completenessを優先して、RECORD / ENRICH RECORD / MISSING EVIDENCEの1つを選び、断定にはsummary・risks・evidenceを添える。実データにない事実を創作せず、不確実な場合は保守的なラベルと人間確認のactionを返す。JSON解析に失敗した場合はfallbackを返す。
...
既存の outputs/14-deployment-black-box-recorder/src/styles.css をデザインSSOTとし、semantic token、既存の色・間隔・フォーカス表示・レスポンシブブレークポイントを再利用する。新規色の追加は判断ラベルの意味が既存表現で不足する場合だけに限定する。

### 0.10 FR Dependency Graph

```
FR-01401 (input/contract) → FR-01402 (AI/execution) → FR-01403 (display)
                                      └──────────────→ FR-01404 (safety/operations)
```
...
Deployment Black Box Recorder によるevidence captureの自動判断を、判断可能なJSONと確認可能なUIとして提供する。

### 1.4 Goals / Non-Goals

**Goals**: evidence capture、timeline anchors、forensic completenessを構造化し、RECORD / ENRICH RECORD / MISSING EVIDENCEと根拠・アクションを返す。Cloud Runで実行可能にする。
...
| FR-ID | 名称 | 要件 | 実装 | テスト |
|---|---|---|---|---|
| FR-01401 | 証拠入力 | target/context/signalsを受け付け、Zodでサイズ・形式を検証する。 | outputs/14-deployment-black-box-recorder/src/server.ts | outputs/14-deployment-black-box-recorder/src/server.test.ts |
| FR-01402 | AI判断 | deployment evidence recorderとしてevidence capture・timeline anchors・forensic completenessをGeminiへ渡し、RECORD / ENRICH RECORD / MISSING EVIDENCEのいずれかを返す。 | outputs/14-deployment-black-box-recorder/src/agent.ts | outputs/14-deployment-black-box-recorder/src/server.test.ts |
| FR-01403 | 結果表示 | confidence、summary、risks、actions、evidence、automationPlan、commentDraftを画面で確認できる。 | outputs/14-deployment-black-box-recorder/src/main.ts | outputs/14-deployment-black-box-recorder/src/server.test.ts |
| FR-01404 | 運用安全性 | Gemini失敗時のfallback、request ID、health/ready/version、認証・rate limit・構造化エラーを提供する。 | outputs/14-deployment-black-box-recorder/src/server.ts | outputs/14-deployment-black-box-recorder/src/server.test.ts |
