# 020-incident-commander-karaoke: Incident Commander Karaoke
...
| 用語 | 定義 | コード表現 |
|---|---|---|
| Agent | incident command rehearsal coachとして判断を行うAI役割 | agent |
| Decision | GOOD COMMAND / COACH / RETRY COMMANDの判断ラベル | decision |
...
| レイヤー | 実体 | 作業 | 条件 |
|---|---|:---:|---|
| Type/Schema | outputs/20-incident-commander-karaoke/src/project.ts | 🔄 | 実装済み型・Zod契約 |
| Type/Schema | outputs/20-incident-commander-karaoke/src/agent.ts | 🔄 | 実装済み型・Zod契約 |
| API/Service | outputs/20-incident-commander-karaoke/src/server.ts | 🔄 | API/外部サービス境界 |
| UI | outputs/20-incident-commander-karaoke/src/main.ts | 🔄 | UIまたは表示層 |
| Test | outputs/20-incident-commander-karaoke/src/server.test.ts | ✅ | 回帰・契約テスト |

#### 0.1.1 Traceability Matrix

| FR-ID | 実装ファイル | テスト | 状態 |
|---|---|---|:---:|
| FR-02001 | outputs/20-incident-commander-karaoke/src/server.ts | outputs/20-incident-commander-karaoke/src/server.test.ts | 実装済み |
| FR-02002 | outputs/20-incident-commander-karaoke/src/agent.ts | outputs/20-incident-commander-karaoke/src/server.test.ts | 実装済み |
| FR-02003 | outputs/20-incident-commander-karaoke/src/main.ts | outputs/20-incident-commander-karaoke/src/server.test.ts | 実装済み |
| FR-02004 | outputs/20-incident-commander-karaoke/src/server.ts | outputs/20-incident-commander-karaoke/src/server.test.ts | 実装済み |
...
システム役割は「incident command rehearsal coach」。入力のcommand clarity・role assignment・communication cadenceを優先して、GOOD COMMAND / COACH / RETRY COMMANDの1つを選び、断定にはsummary・risks・evidenceを添える。実データにない事実を創作せず、不確実な場合は保守的なラベルと人間確認のactionを返す。JSON解析に失敗した場合はfallbackを返す。
...
既存の outputs/20-incident-commander-karaoke/src/styles.css をデザインSSOTとし、semantic token、既存の色・間隔・フォーカス表示・レスポンシブブレークポイントを再利用する。新規色の追加は判断ラベルの意味が既存表現で不足する場合だけに限定する。

### 0.10 FR Dependency Graph

```
FR-02001 (input/contract) → FR-02002 (AI/execution) → FR-02003 (display)
                                      └──────────────→ FR-02004 (safety/operations)
```
...
Incident Commander Karaoke によるcommand clarityの自動判断を、判断可能なJSONと確認可能なUIとして提供する。

### 1.4 Goals / Non-Goals

**Goals**: command clarity、role assignment、communication cadenceを構造化し、GOOD COMMAND / COACH / RETRY COMMANDと根拠・アクションを返す。Cloud Runで実行可能にする。
...
| FR-ID | 名称 | 要件 | 実装 | テスト |
|---|---|---|---|---|
| FR-02001 | 証拠入力 | target/context/signalsを受け付け、Zodでサイズ・形式を検証する。 | outputs/20-incident-commander-karaoke/src/server.ts | outputs/20-incident-commander-karaoke/src/server.test.ts |
| FR-02002 | AI判断 | incident command rehearsal coachとしてcommand clarity・role assignment・communication cadenceをGeminiへ渡し、GOOD COMMAND / COACH / RETRY COMMANDのいずれかを返す。 | outputs/20-incident-commander-karaoke/src/agent.ts | outputs/20-incident-commander-karaoke/src/server.test.ts |
| FR-02003 | 結果表示 | confidence、summary、risks、actions、evidence、automationPlan、commentDraftを画面で確認できる。 | outputs/20-incident-commander-karaoke/src/main.ts | outputs/20-incident-commander-karaoke/src/server.test.ts |
| FR-02004 | 運用安全性 | Gemini失敗時のfallback、request ID、health/ready/version、認証・rate limit・構造化エラーを提供する。 | outputs/20-incident-commander-karaoke/src/server.ts | outputs/20-incident-commander-karaoke/src/server.test.ts |
