# スクリーン: Mission Control

> **画面ID**: SCR-021-mission-control | **関連SPEC**: [SPEC-021-agent-guild-platform](../SPEC-021-agent-guild-platform.md)

## 1. 画面情報

| 項目 | 内容 |
|---|---|
| 実装 | src/AppHome.tsx / src/MissionControl.tsx / src/OpsAgentConsole.tsx |
| ルート | /#mission-control |
| API | GET /api/healthz, POST /api/agent-runs, POST|GET /api/missions, GET /api/agent-runs |
...
| 成功 | RECOMMEND / EXECUTE / VERIFY、confidence、summary、evidence、actions | 根拠を確認/再実行 |
...
- goalまたはtargetを必須または既存schemaの範囲で受け付ける。
- 実行中は二重送信を防止する。
...
| UIイベント | ロジック | 状態変化 |
|---|---|---|
| 実行 | startMission / startAgentRun | idle → loading → data/error |
| 結果取得 | GET /api/missions/:id または /api/agent-runs/:id | loading → data |
...
|---|---|
| 入口 | / |
| 関連導線 | #mission-control, #safe-demo, Agent Card |
| Deep link | N/A - SPA/単一画面MVP |

## 9. ロジックSSOT

- [SPEC-021-agent-guild-platform.md](../SPEC-021-agent-guild-platform.md) §0.2、§0.5、§2
...
- GET /api/market
- POST /api/recommend
- POST /api/agent-card/discover
- GET|POST|DELETE /api/hires
- POST /api/agent-runs
- POST|GET /api/missions
- GET /api/agent-stats
- GET /api/agent-jobs
- GET /api/agent-runs
- POST /api/ops-agent/incident-drill
