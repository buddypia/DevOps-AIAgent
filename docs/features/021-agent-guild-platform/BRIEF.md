# 機能概要: 021-agent-guild-platform-agent-guild-プラットフォーム

> **状態**: 実装済み・仕様化 | **優先度**: P1 | **生成日**: 2026-07-12
> **コンテキスト**: `docs/features/021-agent-guild-platform/CONTEXT.json`
...
Agent Guild プラットフォームは、agent marketplace and squad recommendation・mission orchestration with evidence・Cloud Run SRE operations and A2A integrationに関わる運用判断を、分散した情報から再現可能な判断へ変換する機能です。対象情報を読み解く時間と、判断根拠の共有コストが課題です。
...
| 複雑度 | 中〜高（外部AI・運用判断） |
| ユーザー価値 | A2A DevOps mission controlが判断の初動を短縮し、根拠付きの次アクションを得る |
...
|---|---|---|---|
| US-01 | 目的または運用課題がある | 利用者として、目的に合うエージェントを選びたい | 判断対象を短時間で整理する |
| US-02 | 分析結果を受け取った | 運用担当者として、選んだエージェントに調査を任せたい | 次の行動と責任範囲を説明する |
...
|---|---|---|---|---|
| AC-01 | 入力がスキーマに適合する | 分析を実行する | 構造化結果と判断ラベルが返る | tests/missionAgent.test.ts |
| AC-02 | Geminiが利用できない | 分析を実行する | deterministic fallbackで同じ結果形状を返す | server/opsAgent.ts |
| AC-03 | 結果が返る | 画面を確認する | 信頼度・根拠・次アクション・エラー状態を識別できる | src/OpsAgentConsole.tsx |
...
| リワード | RECOMMEND〜VERIFYの明確な判断と次アクション |
...
| ID | 完了条件 | 検証タイプ | 観測方法 |
|---|---|---|---|
| DoD-FS-01 | 入力検証・分析結果・fallbackが既存テストで確認できる | machine | npm test -- --run tests/missionAgent.test.ts |
| DoD-FS-02 | 判断ラベル、信頼度、根拠、アクション、エラー状態を画面で確認できる | human | AppHome/MissionControl/OpsAgentConsole を目視確認 |
...
| カテゴリ | 参照 |
|---|---|
| コードパターン | src/AppHome.tsx, src/MissionControl.tsx, src/OpsAgentConsole.tsx, src/IncidentDrillPanel.tsx, src/AgentRoster.tsx, src/EvidenceDashboard.tsx、server/index.ts, server/missionAgent.ts, server/opsAgent.ts, server/runStore.ts |
| 型・スキーマ | src/types.ts, src/missionTypes.ts |
| テスト | tests/*.test.ts |
| 仕様・運用 | README.md、docs/02_architecture/a2a-marketplace.md |
