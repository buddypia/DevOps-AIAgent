# 006-deploy-rehearsal-agent: Deploy Rehearsal Agent
...
| 用語 | 定義 | コード表現 |
|---|---|---|
| Agent | pre-production rehearsal directorとして判断を行うAI役割 | agent |
| Decision | READY / REHEARSE / NOT READYの判断ラベル | decision |
...
| レイヤー | 実体 | 作業 | 条件 |
|---|---|:---:|---|
| Type/Schema | outputs/06-deploy-rehearsal-agent/src/project.ts | 🔄 | 実装済み型・Zod契約 |
| Type/Schema | outputs/06-deploy-rehearsal-agent/src/agent.ts | 🔄 | 実装済み型・Zod契約 |
| API/Service | outputs/06-deploy-rehearsal-agent/src/server.ts | 🔄 | API/外部サービス境界 |
| UI | outputs/06-deploy-rehearsal-agent/src/main.ts | 🔄 | UIまたは表示層 |
| Test | outputs/06-deploy-rehearsal-agent/src/server.test.ts | ✅ | 回帰・契約テスト |

#### 0.1.1 Traceability Matrix

| FR-ID | 実装ファイル | テスト | 状態 |
|---|---|---|:---:|
| FR-00601 | outputs/06-deploy-rehearsal-agent/src/server.ts | outputs/06-deploy-rehearsal-agent/src/server.test.ts | 実装済み |
| FR-00602 | outputs/06-deploy-rehearsal-agent/src/agent.ts | outputs/06-deploy-rehearsal-agent/src/server.test.ts | 実装済み |
| FR-00603 | outputs/06-deploy-rehearsal-agent/src/main.ts | outputs/06-deploy-rehearsal-agent/src/server.test.ts | 実装済み |
| FR-00604 | outputs/06-deploy-rehearsal-agent/src/server.ts | outputs/06-deploy-rehearsal-agent/src/server.test.ts | 実装済み |
...
システム役割は「pre-production rehearsal director」。入力のenvironment readiness・command sequencing・migration riskを優先して、READY / REHEARSE / NOT READYの1つを選び、断定にはsummary・risks・evidenceを添える。実データにない事実を創作せず、不確実な場合は保守的なラベルと人間確認のactionを返す。JSON解析に失敗した場合はfallbackを返す。
...
既存の outputs/06-deploy-rehearsal-agent/src/styles.css をデザインSSOTとし、semantic token、既存の色・間隔・フォーカス表示・レスポンシブブレークポイントを再利用する。新規色の追加は判断ラベルの意味が既存表現で不足する場合だけに限定する。

### 0.10 FR Dependency Graph

```
FR-00601 (input/contract) → FR-00602 (AI/execution) → FR-00603 (display)
                                      └──────────────→ FR-00604 (safety/operations)
```
...
Deploy Rehearsal Agent によるenvironment readinessの自動判断を、判断可能なJSONと確認可能なUIとして提供する。

### 1.4 Goals / Non-Goals

**Goals**: environment readiness、command sequencing、migration riskを構造化し、READY / REHEARSE / NOT READYと根拠・アクションを返す。Cloud Runで実行可能にする。
...
| FR-ID | 名称 | 要件 | 実装 | テスト |
|---|---|---|---|---|
| FR-00601 | 証拠入力 | target/context/signalsを受け付け、Zodでサイズ・形式を検証する。 | outputs/06-deploy-rehearsal-agent/src/server.ts | outputs/06-deploy-rehearsal-agent/src/server.test.ts |
| FR-00602 | AI判断 | pre-production rehearsal directorとしてenvironment readiness・command sequencing・migration riskをGeminiへ渡し、READY / REHEARSE / NOT READYのいずれかを返す。 | outputs/06-deploy-rehearsal-agent/src/agent.ts | outputs/06-deploy-rehearsal-agent/src/server.test.ts |
| FR-00603 | 結果表示 | confidence、summary、risks、actions、evidence、automationPlan、commentDraftを画面で確認できる。 | outputs/06-deploy-rehearsal-agent/src/main.ts | outputs/06-deploy-rehearsal-agent/src/server.test.ts |
| FR-00604 | 運用安全性 | Gemini失敗時のfallback、request ID、health/ready/version、認証・rate limit・構造化エラーを提供する。 | outputs/06-deploy-rehearsal-agent/src/server.ts | outputs/06-deploy-rehearsal-agent/src/server.test.ts |
