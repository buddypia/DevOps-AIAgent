# 018-model-rollback-agent: Model Rollback Agent
...
| 用語 | 定義 | コード表現 |
|---|---|---|
| Agent | AI model rollback judgeとして判断を行うAI役割 | agent |
| Decision | KEEP MODEL / CANARY / ROLLBACK MODELの判断ラベル | decision |
...
| レイヤー | 実体 | 作業 | 条件 |
|---|---|:---:|---|
| Type/Schema | outputs/18-model-rollback-agent/src/project.ts | 🔄 | 実装済み型・Zod契約 |
| Type/Schema | outputs/18-model-rollback-agent/src/agent.ts | 🔄 | 実装済み型・Zod契約 |
| API/Service | outputs/18-model-rollback-agent/src/server.ts | 🔄 | API/外部サービス境界 |
| UI | outputs/18-model-rollback-agent/src/main.ts | 🔄 | UIまたは表示層 |
| Test | outputs/18-model-rollback-agent/src/server.test.ts | ✅ | 回帰・契約テスト |

#### 0.1.1 Traceability Matrix

| FR-ID | 実装ファイル | テスト | 状態 |
|---|---|---|:---:|
| FR-01801 | outputs/18-model-rollback-agent/src/server.ts | outputs/18-model-rollback-agent/src/server.test.ts | 実装済み |
| FR-01802 | outputs/18-model-rollback-agent/src/agent.ts | outputs/18-model-rollback-agent/src/server.test.ts | 実装済み |
| FR-01803 | outputs/18-model-rollback-agent/src/main.ts | outputs/18-model-rollback-agent/src/server.test.ts | 実装済み |
| FR-01804 | outputs/18-model-rollback-agent/src/server.ts | outputs/18-model-rollback-agent/src/server.test.ts | 実装済み |
...
システム役割は「AI model rollback judge」。入力のmodel comparison・eval regression・production feedbackを優先して、KEEP MODEL / CANARY / ROLLBACK MODELの1つを選び、断定にはsummary・risks・evidenceを添える。実データにない事実を創作せず、不確実な場合は保守的なラベルと人間確認のactionを返す。JSON解析に失敗した場合はfallbackを返す。
...
既存の outputs/18-model-rollback-agent/src/styles.css をデザインSSOTとし、semantic token、既存の色・間隔・フォーカス表示・レスポンシブブレークポイントを再利用する。新規色の追加は判断ラベルの意味が既存表現で不足する場合だけに限定する。

### 0.10 FR Dependency Graph

```
FR-01801 (input/contract) → FR-01802 (AI/execution) → FR-01803 (display)
                                      └──────────────→ FR-01804 (safety/operations)
```
...
Model Rollback Agent によるmodel comparisonの自動判断を、判断可能なJSONと確認可能なUIとして提供する。

### 1.4 Goals / Non-Goals

**Goals**: model comparison、eval regression、production feedbackを構造化し、KEEP MODEL / CANARY / ROLLBACK MODELと根拠・アクションを返す。Cloud Runで実行可能にする。
...
| FR-ID | 名称 | 要件 | 実装 | テスト |
|---|---|---|---|---|
| FR-01801 | 証拠入力 | target/context/signalsを受け付け、Zodでサイズ・形式を検証する。 | outputs/18-model-rollback-agent/src/server.ts | outputs/18-model-rollback-agent/src/server.test.ts |
| FR-01802 | AI判断 | AI model rollback judgeとしてmodel comparison・eval regression・production feedbackをGeminiへ渡し、KEEP MODEL / CANARY / ROLLBACK MODELのいずれかを返す。 | outputs/18-model-rollback-agent/src/agent.ts | outputs/18-model-rollback-agent/src/server.test.ts |
| FR-01803 | 結果表示 | confidence、summary、risks、actions、evidence、automationPlan、commentDraftを画面で確認できる。 | outputs/18-model-rollback-agent/src/main.ts | outputs/18-model-rollback-agent/src/server.test.ts |
| FR-01804 | 運用安全性 | Gemini失敗時のfallback、request ID、health/ready/version、認証・rate limit・構造化エラーを提供する。 | outputs/18-model-rollback-agent/src/server.ts | outputs/18-model-rollback-agent/src/server.test.ts |
