# 010-cloud-run-traffic-mixer: Cloud Run Traffic Mixer
...
| 用語 | 定義 | コード表現 |
|---|---|---|
| Agent | traffic split planning agentとして判断を行うAI役割 | agent |
| Decision | SHIFT UP / HOLD SPLIT / SHIFT DOWNの判断ラベル | decision |
...
| レイヤー | 実体 | 作業 | 条件 |
|---|---|:---:|---|
| Type/Schema | outputs/10-cloud-run-traffic-mixer/src/project.ts | 🔄 | 実装済み型・Zod契約 |
| Type/Schema | outputs/10-cloud-run-traffic-mixer/src/agent.ts | 🔄 | 実装済み型・Zod契約 |
| API/Service | outputs/10-cloud-run-traffic-mixer/src/server.ts | 🔄 | API/外部サービス境界 |
| UI | outputs/10-cloud-run-traffic-mixer/src/main.ts | 🔄 | UIまたは表示層 |
| Test | outputs/10-cloud-run-traffic-mixer/src/server.test.ts | ✅ | 回帰・契約テスト |

#### 0.1.1 Traceability Matrix

| FR-ID | 実装ファイル | テスト | 状態 |
|---|---|---|:---:|
| FR-01001 | outputs/10-cloud-run-traffic-mixer/src/server.ts | outputs/10-cloud-run-traffic-mixer/src/server.test.ts | 実装済み |
| FR-01002 | outputs/10-cloud-run-traffic-mixer/src/agent.ts | outputs/10-cloud-run-traffic-mixer/src/server.test.ts | 実装済み |
| FR-01003 | outputs/10-cloud-run-traffic-mixer/src/main.ts | outputs/10-cloud-run-traffic-mixer/src/server.test.ts | 実装済み |
| FR-01004 | outputs/10-cloud-run-traffic-mixer/src/server.ts | outputs/10-cloud-run-traffic-mixer/src/server.test.ts | 実装済み |
...
システム役割は「traffic split planning agent」。入力のtraffic stages・wait conditions・rollout safetyを優先して、SHIFT UP / HOLD SPLIT / SHIFT DOWNの1つを選び、断定にはsummary・risks・evidenceを添える。実データにない事実を創作せず、不確実な場合は保守的なラベルと人間確認のactionを返す。JSON解析に失敗した場合はfallbackを返す。
...
既存の outputs/10-cloud-run-traffic-mixer/src/styles.css をデザインSSOTとし、semantic token、既存の色・間隔・フォーカス表示・レスポンシブブレークポイントを再利用する。新規色の追加は判断ラベルの意味が既存表現で不足する場合だけに限定する。

### 0.10 FR Dependency Graph

```
FR-01001 (input/contract) → FR-01002 (AI/execution) → FR-01003 (display)
                                      └──────────────→ FR-01004 (safety/operations)
```
...
Cloud Run Traffic Mixer によるtraffic stagesの自動判断を、判断可能なJSONと確認可能なUIとして提供する。

### 1.4 Goals / Non-Goals

**Goals**: traffic stages、wait conditions、rollout safetyを構造化し、SHIFT UP / HOLD SPLIT / SHIFT DOWNと根拠・アクションを返す。Cloud Runで実行可能にする。
...
| FR-ID | 名称 | 要件 | 実装 | テスト |
|---|---|---|---|---|
| FR-01001 | 証拠入力 | target/context/signalsを受け付け、Zodでサイズ・形式を検証する。 | outputs/10-cloud-run-traffic-mixer/src/server.ts | outputs/10-cloud-run-traffic-mixer/src/server.test.ts |
| FR-01002 | AI判断 | traffic split planning agentとしてtraffic stages・wait conditions・rollout safetyをGeminiへ渡し、SHIFT UP / HOLD SPLIT / SHIFT DOWNのいずれかを返す。 | outputs/10-cloud-run-traffic-mixer/src/agent.ts | outputs/10-cloud-run-traffic-mixer/src/server.test.ts |
| FR-01003 | 結果表示 | confidence、summary、risks、actions、evidence、automationPlan、commentDraftを画面で確認できる。 | outputs/10-cloud-run-traffic-mixer/src/main.ts | outputs/10-cloud-run-traffic-mixer/src/server.test.ts |
| FR-01004 | 運用安全性 | Gemini失敗時のfallback、request ID、health/ready/version、認証・rate limit・構造化エラーを提供する。 | outputs/10-cloud-run-traffic-mixer/src/server.ts | outputs/10-cloud-run-traffic-mixer/src/server.test.ts |
