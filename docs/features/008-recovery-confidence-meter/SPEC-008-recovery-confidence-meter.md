# 008-recovery-confidence-meter: Recovery Confidence Meter
...
| 用語 | 定義 | コード表現 |
|---|---|---|
| Agent | recovery verification agentとして判断を行うAI役割 | agent |
| Decision | CLOSE / VERIFY / KEEP INCIDENT OPENの判断ラベル | decision |
...
| レイヤー | 実体 | 作業 | 条件 |
|---|---|:---:|---|
| Type/Schema | outputs/08-recovery-confidence-meter/src/project.ts | 🔄 | 実装済み型・Zod契約 |
| Type/Schema | outputs/08-recovery-confidence-meter/src/agent.ts | 🔄 | 実装済み型・Zod契約 |
| API/Service | outputs/08-recovery-confidence-meter/src/server.ts | 🔄 | API/外部サービス境界 |
| UI | outputs/08-recovery-confidence-meter/src/main.ts | 🔄 | UIまたは表示層 |
| Test | outputs/08-recovery-confidence-meter/src/server.test.ts | ✅ | 回帰・契約テスト |

#### 0.1.1 Traceability Matrix

| FR-ID | 実装ファイル | テスト | 状態 |
|---|---|---|:---:|
| FR-00801 | outputs/08-recovery-confidence-meter/src/server.ts | outputs/08-recovery-confidence-meter/src/server.test.ts | 実装済み |
| FR-00802 | outputs/08-recovery-confidence-meter/src/agent.ts | outputs/08-recovery-confidence-meter/src/server.test.ts | 実装済み |
| FR-00803 | outputs/08-recovery-confidence-meter/src/main.ts | outputs/08-recovery-confidence-meter/src/server.test.ts | 実装済み |
| FR-00804 | outputs/08-recovery-confidence-meter/src/server.ts | outputs/08-recovery-confidence-meter/src/server.test.ts | 実装済み |
...
システム役割は「recovery verification agent」。入力のbaseline comparison・residual symptoms・closure evidenceを優先して、CLOSE / VERIFY / KEEP INCIDENT OPENの1つを選び、断定にはsummary・risks・evidenceを添える。実データにない事実を創作せず、不確実な場合は保守的なラベルと人間確認のactionを返す。JSON解析に失敗した場合はfallbackを返す。
...
既存の outputs/08-recovery-confidence-meter/src/styles.css をデザインSSOTとし、semantic token、既存の色・間隔・フォーカス表示・レスポンシブブレークポイントを再利用する。新規色の追加は判断ラベルの意味が既存表現で不足する場合だけに限定する。

### 0.10 FR Dependency Graph

```
FR-00801 (input/contract) → FR-00802 (AI/execution) → FR-00803 (display)
                                      └──────────────→ FR-00804 (safety/operations)
```
...
Recovery Confidence Meter によるbaseline comparisonの自動判断を、判断可能なJSONと確認可能なUIとして提供する。

### 1.4 Goals / Non-Goals

**Goals**: baseline comparison、residual symptoms、closure evidenceを構造化し、CLOSE / VERIFY / KEEP INCIDENT OPENと根拠・アクションを返す。Cloud Runで実行可能にする。
...
| FR-ID | 名称 | 要件 | 実装 | テスト |
|---|---|---|---|---|
| FR-00801 | 証拠入力 | target/context/signalsを受け付け、Zodでサイズ・形式を検証する。 | outputs/08-recovery-confidence-meter/src/server.ts | outputs/08-recovery-confidence-meter/src/server.test.ts |
| FR-00802 | AI判断 | recovery verification agentとしてbaseline comparison・residual symptoms・closure evidenceをGeminiへ渡し、CLOSE / VERIFY / KEEP INCIDENT OPENのいずれかを返す。 | outputs/08-recovery-confidence-meter/src/agent.ts | outputs/08-recovery-confidence-meter/src/server.test.ts |
| FR-00803 | 結果表示 | confidence、summary、risks、actions、evidence、automationPlan、commentDraftを画面で確認できる。 | outputs/08-recovery-confidence-meter/src/main.ts | outputs/08-recovery-confidence-meter/src/server.test.ts |
| FR-00804 | 運用安全性 | Gemini失敗時のfallback、request ID、health/ready/version、認証・rate limit・構造化エラーを提供する。 | outputs/08-recovery-confidence-meter/src/server.ts | outputs/08-recovery-confidence-meter/src/server.test.ts |
