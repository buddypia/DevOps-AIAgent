# 007-post-deploy-judge: Post Deploy Judge
...
| 用語 | 定義 | コード表現 |
|---|---|---|
| Agent | post-deployment decision agentとして判断を行うAI役割 | agent |
| Decision | SUCCESS / WATCH / ROLLBACKの判断ラベル | decision |
...
| レイヤー | 実体 | 作業 | 条件 |
|---|---|:---:|---|
| Type/Schema | outputs/07-post-deploy-judge/src/project.ts | 🔄 | 実装済み型・Zod契約 |
| Type/Schema | outputs/07-post-deploy-judge/src/agent.ts | 🔄 | 実装済み型・Zod契約 |
| API/Service | outputs/07-post-deploy-judge/src/server.ts | 🔄 | API/外部サービス境界 |
| UI | outputs/07-post-deploy-judge/src/main.ts | 🔄 | UIまたは表示層 |
| Test | outputs/07-post-deploy-judge/src/server.test.ts | ✅ | 回帰・契約テスト |

#### 0.1.1 Traceability Matrix

| FR-ID | 実装ファイル | テスト | 状態 |
|---|---|---|:---:|
| FR-00701 | outputs/07-post-deploy-judge/src/server.ts | outputs/07-post-deploy-judge/src/server.test.ts | 実装済み |
| FR-00702 | outputs/07-post-deploy-judge/src/agent.ts | outputs/07-post-deploy-judge/src/server.test.ts | 実装済み |
| FR-00703 | outputs/07-post-deploy-judge/src/main.ts | outputs/07-post-deploy-judge/src/server.test.ts | 実装済み |
| FR-00704 | outputs/07-post-deploy-judge/src/server.ts | outputs/07-post-deploy-judge/src/server.test.ts | 実装済み |
...
システム役割は「post-deployment decision agent」。入力のpost-deploy window・success criteria・regression watchを優先して、SUCCESS / WATCH / ROLLBACKの1つを選び、断定にはsummary・risks・evidenceを添える。実データにない事実を創作せず、不確実な場合は保守的なラベルと人間確認のactionを返す。JSON解析に失敗した場合はfallbackを返す。
...
既存の outputs/07-post-deploy-judge/src/styles.css をデザインSSOTとし、semantic token、既存の色・間隔・フォーカス表示・レスポンシブブレークポイントを再利用する。新規色の追加は判断ラベルの意味が既存表現で不足する場合だけに限定する。

### 0.10 FR Dependency Graph

```
FR-00701 (input/contract) → FR-00702 (AI/execution) → FR-00703 (display)
                                      └──────────────→ FR-00704 (safety/operations)
```
...
Post Deploy Judge によるpost-deploy windowの自動判断を、判断可能なJSONと確認可能なUIとして提供する。

### 1.4 Goals / Non-Goals

**Goals**: post-deploy window、success criteria、regression watchを構造化し、SUCCESS / WATCH / ROLLBACKと根拠・アクションを返す。Cloud Runで実行可能にする。
...
| FR-ID | 名称 | 要件 | 実装 | テスト |
|---|---|---|---|---|
| FR-00701 | 証拠入力 | target/context/signalsを受け付け、Zodでサイズ・形式を検証する。 | outputs/07-post-deploy-judge/src/server.ts | outputs/07-post-deploy-judge/src/server.test.ts |
| FR-00702 | AI判断 | post-deployment decision agentとしてpost-deploy window・success criteria・regression watchをGeminiへ渡し、SUCCESS / WATCH / ROLLBACKのいずれかを返す。 | outputs/07-post-deploy-judge/src/agent.ts | outputs/07-post-deploy-judge/src/server.test.ts |
| FR-00703 | 結果表示 | confidence、summary、risks、actions、evidence、automationPlan、commentDraftを画面で確認できる。 | outputs/07-post-deploy-judge/src/main.ts | outputs/07-post-deploy-judge/src/server.test.ts |
| FR-00704 | 運用安全性 | Gemini失敗時のfallback、request ID、health/ready/version、認証・rate limit・構造化エラーを提供する。 | outputs/07-post-deploy-judge/src/server.ts | outputs/07-post-deploy-judge/src/server.test.ts |
