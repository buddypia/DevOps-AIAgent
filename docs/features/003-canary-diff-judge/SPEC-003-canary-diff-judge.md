# 003-canary-diff-judge: Canary Diff Judge
...
| 用語 | 定義 | コード表現 |
|---|---|---|
| Agent | canary promotion judgeとして判断を行うAI役割 | agent |
| Decision | PROMOTE / HOLD / ROLLBACKの判断ラベル | decision |
...
| レイヤー | 実体 | 作業 | 条件 |
|---|---|:---:|---|
| Type/Schema | outputs/03-canary-diff-judge/src/project.ts | 🔄 | 実装済み型・Zod契約 |
| Type/Schema | outputs/03-canary-diff-judge/src/agent.ts | 🔄 | 実装済み型・Zod契約 |
| API/Service | outputs/03-canary-diff-judge/src/server.ts | 🔄 | API/外部サービス境界 |
| UI | outputs/03-canary-diff-judge/src/main.ts | 🔄 | UIまたは表示層 |
| Test | outputs/03-canary-diff-judge/src/server.test.ts | ✅ | 回帰・契約テスト |

#### 0.1.1 Traceability Matrix

| FR-ID | 実装ファイル | テスト | 状態 |
|---|---|---|:---:|
| FR-00301 | outputs/03-canary-diff-judge/src/server.ts | outputs/03-canary-diff-judge/src/server.test.ts | 実装済み |
| FR-00302 | outputs/03-canary-diff-judge/src/agent.ts | outputs/03-canary-diff-judge/src/server.test.ts | 実装済み |
| FR-00303 | outputs/03-canary-diff-judge/src/main.ts | outputs/03-canary-diff-judge/src/server.test.ts | 実装済み |
| FR-00304 | outputs/03-canary-diff-judge/src/server.ts | outputs/03-canary-diff-judge/src/server.test.ts | 実装済み |
...
システム役割は「canary promotion judge」。入力のrevision comparison・traffic split・rollback thresholdを優先して、PROMOTE / HOLD / ROLLBACKの1つを選び、断定にはsummary・risks・evidenceを添える。実データにない事実を創作せず、不確実な場合は保守的なラベルと人間確認のactionを返す。JSON解析に失敗した場合はfallbackを返す。
...
既存の outputs/03-canary-diff-judge/src/styles.css をデザインSSOTとし、semantic token、既存の色・間隔・フォーカス表示・レスポンシブブレークポイントを再利用する。新規色の追加は判断ラベルの意味が既存表現で不足する場合だけに限定する。

### 0.10 FR Dependency Graph

```
FR-00301 (input/contract) → FR-00302 (AI/execution) → FR-00303 (display)
                                      └──────────────→ FR-00304 (safety/operations)
```
...
Canary Diff Judge によるrevision comparisonの自動判断を、判断可能なJSONと確認可能なUIとして提供する。

### 1.4 Goals / Non-Goals

**Goals**: revision comparison、traffic split、rollback thresholdを構造化し、PROMOTE / HOLD / ROLLBACKと根拠・アクションを返す。Cloud Runで実行可能にする。
...
| FR-ID | 名称 | 要件 | 実装 | テスト |
|---|---|---|---|---|
| FR-00301 | 証拠入力 | target/context/signalsを受け付け、Zodでサイズ・形式を検証する。 | outputs/03-canary-diff-judge/src/server.ts | outputs/03-canary-diff-judge/src/server.test.ts |
| FR-00302 | AI判断 | canary promotion judgeとしてrevision comparison・traffic split・rollback thresholdをGeminiへ渡し、PROMOTE / HOLD / ROLLBACKのいずれかを返す。 | outputs/03-canary-diff-judge/src/agent.ts | outputs/03-canary-diff-judge/src/server.test.ts |
| FR-00303 | 結果表示 | confidence、summary、risks、actions、evidence、automationPlan、commentDraftを画面で確認できる。 | outputs/03-canary-diff-judge/src/main.ts | outputs/03-canary-diff-judge/src/server.test.ts |
| FR-00304 | 運用安全性 | Gemini失敗時のfallback、request ID、health/ready/version、認証・rate limit・構造化エラーを提供する。 | outputs/03-canary-diff-judge/src/server.ts | outputs/03-canary-diff-judge/src/server.test.ts |
