# 機能概要: 004-blast-radius-agent-blast-radius-agent

> **状態**: 実装済み・仕様化 | **優先度**: P1 | **生成日**: 2026-07-12
> **コンテキスト**: `docs/features/004-blast-radius-agent/CONTEXT.json`
...
Blast Radius Agentは、affected surfaces・review routing・test gap hintsに関わる運用判断を、分散した情報から再現可能な判断へ変換する機能です。対象情報を読み解く時間と、判断根拠の共有コストが課題です。
...
| 複雑度 | 中〜高（外部AI・運用判断） |
| ユーザー価値 | change impact forecasterが判断の初動を短縮し、根拠付きの次アクションを得る |
...
|---|---|---|---|
| US-01 | リリース・障害・品質判断が必要 | 運用担当者として、Blast Radius Agentに対象の証拠を渡したい | 判断対象を短時間で整理する |
| US-02 | 分析結果を受け取った | リリース責任者として、Blast Radius AgentのLOW RADIUS / WATCH RADIUS / HIGH RADIUSの判断を根拠付きで確認したい | 次の行動と責任範囲を説明する |
...
|---|---|---|---|---|
| AC-01 | 入力がスキーマに適合する | 分析を実行する | 構造化結果と判断ラベルが返る | outputs/04-blast-radius-agent/src/server.test.ts |
| AC-02 | Geminiが利用できない | 分析を実行する | deterministic fallbackで同じ結果形状を返す | outputs/04-blast-radius-agent/src/agent.ts |
| AC-03 | 結果が返る | 画面を確認する | 信頼度・根拠・次アクション・エラー状態を識別できる | outputs/04-blast-radius-agent/src/main.ts |
...
| リワード | LOW RADIUS〜HIGH RADIUSの明確な判断と次アクション |
...
| ID | 完了条件 | 検証タイプ | 観測方法 |
|---|---|---|---|
| DoD-FS-01 | 入力検証・分析結果・fallbackが既存テストで確認できる | machine | npm test -- --run outputs/04-blast-radius-agent/src/server.test.ts |
| DoD-FS-02 | 判断ラベル、信頼度、根拠、アクション、エラー状態を画面で確認できる | human | outputs/04-blast-radius-agent/src/main.ts を目視確認 |
...
| カテゴリ | 参照 |
|---|---|
| コードパターン | outputs/04-blast-radius-agent/src/main.ts、outputs/04-blast-radius-agent/src/server.ts |
| 型・スキーマ | outputs/04-blast-radius-agent/src/project.ts, outputs/04-blast-radius-agent/src/agent.ts |
| テスト | outputs/04-blast-radius-agent/src/server.test.ts |
| 仕様・運用 | outputs/04-blast-radius-agent/README.md、outputs/04-blast-radius-agent/docs/development.html |
