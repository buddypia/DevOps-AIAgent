# 004-blast-radius-agent: Blast Radius Agent
...
| 用語 | 定義 | コード表現 |
|---|---|---|
| Agent | change impact forecasterとして判断を行うAI役割 | agent |
| Decision | LOW RADIUS / WATCH RADIUS / HIGH RADIUSの判断ラベル | decision |
...
| レイヤー | 実体 | 作業 | 条件 |
|---|---|:---:|---|
| Type/Schema | outputs/04-blast-radius-agent/src/project.ts | 🔄 | 実装済み型・Zod契約 |
| Type/Schema | outputs/04-blast-radius-agent/src/agent.ts | 🔄 | 実装済み型・Zod契約 |
| API/Service | outputs/04-blast-radius-agent/src/server.ts | 🔄 | API/外部サービス境界 |
| UI | outputs/04-blast-radius-agent/src/main.ts | 🔄 | UIまたは表示層 |
| Test | outputs/04-blast-radius-agent/src/server.test.ts | ✅ | 回帰・契約テスト |

#### 0.1.1 Traceability Matrix

| FR-ID | 実装ファイル | テスト | 状態 |
|---|---|---|:---:|
| FR-00401 | outputs/04-blast-radius-agent/src/server.ts | outputs/04-blast-radius-agent/src/server.test.ts | 実装済み |
| FR-00402 | outputs/04-blast-radius-agent/src/agent.ts | outputs/04-blast-radius-agent/src/server.test.ts | 実装済み |
| FR-00403 | outputs/04-blast-radius-agent/src/main.ts | outputs/04-blast-radius-agent/src/server.test.ts | 実装済み |
| FR-00404 | outputs/04-blast-radius-agent/src/server.ts | outputs/04-blast-radius-agent/src/server.test.ts | 実装済み |
...
システム役割は「change impact forecaster」。入力のaffected surfaces・review routing・test gap hintsを優先して、LOW RADIUS / WATCH RADIUS / HIGH RADIUSの1つを選び、断定にはsummary・risks・evidenceを添える。実データにない事実を創作せず、不確実な場合は保守的なラベルと人間確認のactionを返す。JSON解析に失敗した場合はfallbackを返す。
...
既存の outputs/04-blast-radius-agent/src/styles.css をデザインSSOTとし、semantic token、既存の色・間隔・フォーカス表示・レスポンシブブレークポイントを再利用する。新規色の追加は判断ラベルの意味が既存表現で不足する場合だけに限定する。

### 0.10 FR Dependency Graph

```
FR-00401 (input/contract) → FR-00402 (AI/execution) → FR-00403 (display)
                                      └──────────────→ FR-00404 (safety/operations)
```
...
Blast Radius Agent によるaffected surfacesの自動判断を、判断可能なJSONと確認可能なUIとして提供する。

### 1.4 Goals / Non-Goals

**Goals**: affected surfaces、review routing、test gap hintsを構造化し、LOW RADIUS / WATCH RADIUS / HIGH RADIUSと根拠・アクションを返す。Cloud Runで実行可能にする。
...
| FR-ID | 名称 | 要件 | 実装 | テスト |
|---|---|---|---|---|
| FR-00401 | 証拠入力 | target/context/signalsを受け付け、Zodでサイズ・形式を検証する。 | outputs/04-blast-radius-agent/src/server.ts | outputs/04-blast-radius-agent/src/server.test.ts |
| FR-00402 | AI判断 | change impact forecasterとしてaffected surfaces・review routing・test gap hintsをGeminiへ渡し、LOW RADIUS / WATCH RADIUS / HIGH RADIUSのいずれかを返す。 | outputs/04-blast-radius-agent/src/agent.ts | outputs/04-blast-radius-agent/src/server.test.ts |
| FR-00403 | 結果表示 | confidence、summary、risks、actions、evidence、automationPlan、commentDraftを画面で確認できる。 | outputs/04-blast-radius-agent/src/main.ts | outputs/04-blast-radius-agent/src/server.test.ts |
| FR-00404 | 運用安全性 | Gemini失敗時のfallback、request ID、health/ready/version、認証・rate limit・構造化エラーを提供する。 | outputs/04-blast-radius-agent/src/server.ts | outputs/04-blast-radius-agent/src/server.test.ts |
