# 019-chaos-drill-agent: Chaos Drill Agent
...
| 用語 | 定義 | コード表現 |
|---|---|---|
| Agent | chaos exercise conductorとして判断を行うAI役割 | agent |
| Decision | RUN DRILL / TIGHTEN GUARDRAILS / DO NOT RUNの判断ラベル | decision |
...
| レイヤー | 実体 | 作業 | 条件 |
|---|---|:---:|---|
| Type/Schema | outputs/19-chaos-drill-agent/src/project.ts | 🔄 | 実装済み型・Zod契約 |
| Type/Schema | outputs/19-chaos-drill-agent/src/agent.ts | 🔄 | 実装済み型・Zod契約 |
| API/Service | outputs/19-chaos-drill-agent/src/server.ts | 🔄 | API/外部サービス境界 |
| UI | outputs/19-chaos-drill-agent/src/main.ts | 🔄 | UIまたは表示層 |
| Test | outputs/19-chaos-drill-agent/src/server.test.ts | ✅ | 回帰・契約テスト |

#### 0.1.1 Traceability Matrix

| FR-ID | 実装ファイル | テスト | 状態 |
|---|---|---|:---:|
| FR-01901 | outputs/19-chaos-drill-agent/src/server.ts | outputs/19-chaos-drill-agent/src/server.test.ts | 実装済み |
| FR-01902 | outputs/19-chaos-drill-agent/src/agent.ts | outputs/19-chaos-drill-agent/src/server.test.ts | 実装済み |
| FR-01903 | outputs/19-chaos-drill-agent/src/main.ts | outputs/19-chaos-drill-agent/src/server.test.ts | 実装済み |
| FR-01904 | outputs/19-chaos-drill-agent/src/server.ts | outputs/19-chaos-drill-agent/src/server.test.ts | 実装済み |
...
システム役割は「chaos exercise conductor」。入力のfailure injection・safety guardrails・response scoringを優先して、RUN DRILL / TIGHTEN GUARDRAILS / DO NOT RUNの1つを選び、断定にはsummary・risks・evidenceを添える。実データにない事実を創作せず、不確実な場合は保守的なラベルと人間確認のactionを返す。JSON解析に失敗した場合はfallbackを返す。
...
既存の outputs/19-chaos-drill-agent/src/styles.css をデザインSSOTとし、semantic token、既存の色・間隔・フォーカス表示・レスポンシブブレークポイントを再利用する。新規色の追加は判断ラベルの意味が既存表現で不足する場合だけに限定する。

### 0.10 FR Dependency Graph

```
FR-01901 (input/contract) → FR-01902 (AI/execution) → FR-01903 (display)
                                      └──────────────→ FR-01904 (safety/operations)
```
...
Chaos Drill Agent によるfailure injectionの自動判断を、判断可能なJSONと確認可能なUIとして提供する。

### 1.4 Goals / Non-Goals

**Goals**: failure injection、safety guardrails、response scoringを構造化し、RUN DRILL / TIGHTEN GUARDRAILS / DO NOT RUNと根拠・アクションを返す。Cloud Runで実行可能にする。
...
| FR-ID | 名称 | 要件 | 実装 | テスト |
|---|---|---|---|---|
| FR-01901 | 証拠入力 | target/context/signalsを受け付け、Zodでサイズ・形式を検証する。 | outputs/19-chaos-drill-agent/src/server.ts | outputs/19-chaos-drill-agent/src/server.test.ts |
| FR-01902 | AI判断 | chaos exercise conductorとしてfailure injection・safety guardrails・response scoringをGeminiへ渡し、RUN DRILL / TIGHTEN GUARDRAILS / DO NOT RUNのいずれかを返す。 | outputs/19-chaos-drill-agent/src/agent.ts | outputs/19-chaos-drill-agent/src/server.test.ts |
| FR-01903 | 結果表示 | confidence、summary、risks、actions、evidence、automationPlan、commentDraftを画面で確認できる。 | outputs/19-chaos-drill-agent/src/main.ts | outputs/19-chaos-drill-agent/src/server.test.ts |
| FR-01904 | 運用安全性 | Gemini失敗時のfallback、request ID、health/ready/version、認証・rate limit・構造化エラーを提供する。 | outputs/19-chaos-drill-agent/src/server.ts | outputs/19-chaos-drill-agent/src/server.test.ts |
