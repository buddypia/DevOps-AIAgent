# 005-privacy-impact-diff-agent: Privacy Impact Diff Agent
...
| 用語 | 定義 | コード表現 |
|---|---|---|
| Agent | privacy diff reviewerとして判断を行うAI役割 | agent |
| Decision | CLEAR / REVIEW / BLOCK PRIVACYの判断ラベル | decision |
...
| レイヤー | 実体 | 作業 | 条件 |
|---|---|:---:|---|
| Type/Schema | outputs/05-privacy-impact-diff-agent/src/project.ts | 🔄 | 実装済み型・Zod契約 |
| Type/Schema | outputs/05-privacy-impact-diff-agent/src/agent.ts | 🔄 | 実装済み型・Zod契約 |
| API/Service | outputs/05-privacy-impact-diff-agent/src/server.ts | 🔄 | API/外部サービス境界 |
| UI | outputs/05-privacy-impact-diff-agent/src/main.ts | 🔄 | UIまたは表示層 |
| Test | outputs/05-privacy-impact-diff-agent/src/server.test.ts | ✅ | 回帰・契約テスト |

#### 0.1.1 Traceability Matrix

| FR-ID | 実装ファイル | テスト | 状態 |
|---|---|---|:---:|
| FR-00501 | outputs/05-privacy-impact-diff-agent/src/server.ts | outputs/05-privacy-impact-diff-agent/src/server.test.ts | 実装済み |
| FR-00502 | outputs/05-privacy-impact-diff-agent/src/agent.ts | outputs/05-privacy-impact-diff-agent/src/server.test.ts | 実装済み |
| FR-00503 | outputs/05-privacy-impact-diff-agent/src/main.ts | outputs/05-privacy-impact-diff-agent/src/server.test.ts | 実装済み |
| FR-00504 | outputs/05-privacy-impact-diff-agent/src/server.ts | outputs/05-privacy-impact-diff-agent/src/server.test.ts | 実装済み |
...
システム役割は「privacy diff reviewer」。入力のPII detection・log exposure・external sharingを優先して、CLEAR / REVIEW / BLOCK PRIVACYの1つを選び、断定にはsummary・risks・evidenceを添える。実データにない事実を創作せず、不確実な場合は保守的なラベルと人間確認のactionを返す。JSON解析に失敗した場合はfallbackを返す。
...
- PIIの可能性を保守的に扱い、CLEARでも人間レビューを要求する。

### 0.9 Design Tokens

既存の outputs/05-privacy-impact-diff-agent/src/styles.css をデザインSSOTとし、semantic token、既存の色・間隔・フォーカス表示・レスポンシブブレークポイントを再利用する。新規色の追加は判断ラベルの意味が既存表現で不足する場合だけに限定する。

### 0.10 FR Dependency Graph

```
FR-00501 (input/contract) → FR-00502 (AI/execution) → FR-00503 (display)
                                      └──────────────→ FR-00504 (safety/operations)
```
...
Privacy Impact Diff Agent によるPII detectionの自動判断を、判断可能なJSONと確認可能なUIとして提供する。

### 1.4 Goals / Non-Goals

**Goals**: PII detection、log exposure、external sharingを構造化し、CLEAR / REVIEW / BLOCK PRIVACYと根拠・アクションを返す。Cloud Runで実行可能にする。
...
| FR-ID | 名称 | 要件 | 実装 | テスト |
|---|---|---|---|---|
| FR-00501 | 証拠入力 | target/context/signalsを受け付け、Zodでサイズ・形式を検証する。 | outputs/05-privacy-impact-diff-agent/src/server.ts | outputs/05-privacy-impact-diff-agent/src/server.test.ts |
| FR-00502 | AI判断 | privacy diff reviewerとしてPII detection・log exposure・external sharingをGeminiへ渡し、CLEAR / REVIEW / BLOCK PRIVACYのいずれかを返す。 | outputs/05-privacy-impact-diff-agent/src/agent.ts | outputs/05-privacy-impact-diff-agent/src/server.test.ts |
| FR-00503 | 結果表示 | confidence、summary、risks、actions、evidence、automationPlan、commentDraftを画面で確認できる。 | outputs/05-privacy-impact-diff-agent/src/main.ts | outputs/05-privacy-impact-diff-agent/src/server.test.ts |
| FR-00504 | 運用安全性 | Gemini失敗時のfallback、request ID、health/ready/version、認証・rate limit・構造化エラーを提供する。 | outputs/05-privacy-impact-diff-agent/src/server.ts | outputs/05-privacy-impact-diff-agent/src/server.test.ts |
