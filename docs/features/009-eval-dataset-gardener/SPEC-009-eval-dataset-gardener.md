# 009-eval-dataset-gardener: Eval Dataset Gardener
...
| 用語 | 定義 | コード表現 |
|---|---|---|
| Agent | AI eval maintenance agentとして判断を行うAI役割 | agent |
| Decision | ADD CASES / CURATE / BLOCK MODEL CHANGEの判断ラベル | decision |
...
| レイヤー | 実体 | 作業 | 条件 |
|---|---|:---:|---|
| Type/Schema | outputs/09-eval-dataset-gardener/src/project.ts | 🔄 | 実装済み型・Zod契約 |
| Type/Schema | outputs/09-eval-dataset-gardener/src/agent.ts | 🔄 | 実装済み型・Zod契約 |
| API/Service | outputs/09-eval-dataset-gardener/src/server.ts | 🔄 | API/外部サービス境界 |
| UI | outputs/09-eval-dataset-gardener/src/main.ts | 🔄 | UIまたは表示層 |
| Test | outputs/09-eval-dataset-gardener/src/server.test.ts | ✅ | 回帰・契約テスト |

#### 0.1.1 Traceability Matrix

| FR-ID | 実装ファイル | テスト | 状態 |
|---|---|---|:---:|
| FR-00901 | outputs/09-eval-dataset-gardener/src/server.ts | outputs/09-eval-dataset-gardener/src/server.test.ts | 実装済み |
| FR-00902 | outputs/09-eval-dataset-gardener/src/agent.ts | outputs/09-eval-dataset-gardener/src/server.test.ts | 実装済み |
| FR-00903 | outputs/09-eval-dataset-gardener/src/main.ts | outputs/09-eval-dataset-gardener/src/server.test.ts | 実装済み |
| FR-00904 | outputs/09-eval-dataset-gardener/src/server.ts | outputs/09-eval-dataset-gardener/src/server.test.ts | 実装済み |
...
システム役割は「AI eval maintenance agent」。入力のfailure harvesting・expected answer drafting・regression coverageを優先して、ADD CASES / CURATE / BLOCK MODEL CHANGEの1つを選び、断定にはsummary・risks・evidenceを添える。実データにない事実を創作せず、不確実な場合は保守的なラベルと人間確認のactionを返す。JSON解析に失敗した場合はfallbackを返す。
...
既存の outputs/09-eval-dataset-gardener/src/styles.css をデザインSSOTとし、semantic token、既存の色・間隔・フォーカス表示・レスポンシブブレークポイントを再利用する。新規色の追加は判断ラベルの意味が既存表現で不足する場合だけに限定する。

### 0.10 FR Dependency Graph

```
FR-00901 (input/contract) → FR-00902 (AI/execution) → FR-00903 (display)
                                      └──────────────→ FR-00904 (safety/operations)
```
...
Eval Dataset Gardener によるfailure harvestingの自動判断を、判断可能なJSONと確認可能なUIとして提供する。

### 1.4 Goals / Non-Goals

**Goals**: failure harvesting、expected answer drafting、regression coverageを構造化し、ADD CASES / CURATE / BLOCK MODEL CHANGEと根拠・アクションを返す。Cloud Runで実行可能にする。
...
| FR-ID | 名称 | 要件 | 実装 | テスト |
|---|---|---|---|---|
| FR-00901 | 証拠入力 | target/context/signalsを受け付け、Zodでサイズ・形式を検証する。 | outputs/09-eval-dataset-gardener/src/server.ts | outputs/09-eval-dataset-gardener/src/server.test.ts |
| FR-00902 | AI判断 | AI eval maintenance agentとしてfailure harvesting・expected answer drafting・regression coverageをGeminiへ渡し、ADD CASES / CURATE / BLOCK MODEL CHANGEのいずれかを返す。 | outputs/09-eval-dataset-gardener/src/agent.ts | outputs/09-eval-dataset-gardener/src/server.test.ts |
| FR-00903 | 結果表示 | confidence、summary、risks、actions、evidence、automationPlan、commentDraftを画面で確認できる。 | outputs/09-eval-dataset-gardener/src/main.ts | outputs/09-eval-dataset-gardener/src/server.test.ts |
| FR-00904 | 運用安全性 | Gemini失敗時のfallback、request ID、health/ready/version、認証・rate limit・構造化エラーを提供する。 | outputs/09-eval-dataset-gardener/src/server.ts | outputs/09-eval-dataset-gardener/src/server.test.ts |
