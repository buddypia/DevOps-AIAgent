# 002-two-minute-triage: Two Minute Triage
...
| 用語 | 定義 | コード表現 |
|---|---|---|
| Agent | incident first-response agentとして判断を行うAI役割 | agent |
| Decision | STABILIZE / INVESTIGATE / ESCALATEの判断ラベル | decision |
...
| レイヤー | 実体 | 作業 | 条件 |
|---|---|:---:|---|
| Type/Schema | outputs/02-two-minute-triage/src/project.ts | 🔄 | 実装済み型・Zod契約 |
| Type/Schema | outputs/02-two-minute-triage/src/agent.ts | 🔄 | 実装済み型・Zod契約 |
| API/Service | outputs/02-two-minute-triage/src/server.ts | 🔄 | API/外部サービス境界 |
| UI | outputs/02-two-minute-triage/src/main.ts | 🔄 | UIまたは表示層 |
| Test | outputs/02-two-minute-triage/src/server.test.ts | ✅ | 回帰・契約テスト |

#### 0.1.1 Traceability Matrix

| FR-ID | 実装ファイル | テスト | 状態 |
|---|---|---|:---:|
| FR-00201 | outputs/02-two-minute-triage/src/server.ts | outputs/02-two-minute-triage/src/server.test.ts | 実装済み |
| FR-00202 | outputs/02-two-minute-triage/src/agent.ts | outputs/02-two-minute-triage/src/server.test.ts | 実装済み |
| FR-00203 | outputs/02-two-minute-triage/src/main.ts | outputs/02-two-minute-triage/src/server.test.ts | 実装済み |
| FR-00204 | outputs/02-two-minute-triage/src/server.ts | outputs/02-two-minute-triage/src/server.test.ts | 実装済み |
...
システム役割は「incident first-response agent」。入力のfirst signal selection・owner routing・noise reductionを優先して、STABILIZE / INVESTIGATE / ESCALATEの1つを選び、断定にはsummary・risks・evidenceを添える。実データにない事実を創作せず、不確実な場合は保守的なラベルと人間確認のactionを返す。JSON解析に失敗した場合はfallbackを返す。
...
既存の outputs/02-two-minute-triage/src/styles.css をデザインSSOTとし、semantic token、既存の色・間隔・フォーカス表示・レスポンシブブレークポイントを再利用する。新規色の追加は判断ラベルの意味が既存表現で不足する場合だけに限定する。

### 0.10 FR Dependency Graph

```
FR-00201 (input/contract) → FR-00202 (AI/execution) → FR-00203 (display)
                                      └──────────────→ FR-00204 (safety/operations)
```
...
Two Minute Triage によるfirst signal selectionの自動判断を、判断可能なJSONと確認可能なUIとして提供する。

### 1.4 Goals / Non-Goals

**Goals**: first signal selection、owner routing、noise reductionを構造化し、STABILIZE / INVESTIGATE / ESCALATEと根拠・アクションを返す。Cloud Runで実行可能にする。
...
| FR-ID | 名称 | 要件 | 実装 | テスト |
|---|---|---|---|---|
| FR-00201 | 証拠入力 | target/context/signalsを受け付け、Zodでサイズ・形式を検証する。 | outputs/02-two-minute-triage/src/server.ts | outputs/02-two-minute-triage/src/server.test.ts |
| FR-00202 | AI判断 | incident first-response agentとしてfirst signal selection・owner routing・noise reductionをGeminiへ渡し、STABILIZE / INVESTIGATE / ESCALATEのいずれかを返す。 | outputs/02-two-minute-triage/src/agent.ts | outputs/02-two-minute-triage/src/server.test.ts |
| FR-00203 | 結果表示 | confidence、summary、risks、actions、evidence、automationPlan、commentDraftを画面で確認できる。 | outputs/02-two-minute-triage/src/main.ts | outputs/02-two-minute-triage/src/server.test.ts |
| FR-00204 | 運用安全性 | Gemini失敗時のfallback、request ID、health/ready/version、認証・rate limit・構造化エラーを提供する。 | outputs/02-two-minute-triage/src/server.ts | outputs/02-two-minute-triage/src/server.test.ts |
