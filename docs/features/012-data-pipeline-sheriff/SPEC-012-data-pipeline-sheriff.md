# 012-data-pipeline-sheriff: Data Pipeline Sheriff
...
| 用語 | 定義 | コード表現 |
|---|---|---|
| Agent | data pipeline operations agentとして判断を行うAI役割 | agent |
| Decision | RERUN / HOLD DATA / ESCALATEの判断ラベル | decision |
...
| レイヤー | 実体 | 作業 | 条件 |
|---|---|:---:|---|
| Type/Schema | outputs/12-data-pipeline-sheriff/src/project.ts | 🔄 | 実装済み型・Zod契約 |
| Type/Schema | outputs/12-data-pipeline-sheriff/src/agent.ts | 🔄 | 実装済み型・Zod契約 |
| API/Service | outputs/12-data-pipeline-sheriff/src/server.ts | 🔄 | API/外部サービス境界 |
| UI | outputs/12-data-pipeline-sheriff/src/main.ts | 🔄 | UIまたは表示層 |
| Test | outputs/12-data-pipeline-sheriff/src/server.test.ts | ✅ | 回帰・契約テスト |

#### 0.1.1 Traceability Matrix

| FR-ID | 実装ファイル | テスト | 状態 |
|---|---|---|:---:|
| FR-01201 | outputs/12-data-pipeline-sheriff/src/server.ts | outputs/12-data-pipeline-sheriff/src/server.test.ts | 実装済み |
| FR-01202 | outputs/12-data-pipeline-sheriff/src/agent.ts | outputs/12-data-pipeline-sheriff/src/server.test.ts | 実装済み |
| FR-01203 | outputs/12-data-pipeline-sheriff/src/main.ts | outputs/12-data-pipeline-sheriff/src/server.test.ts | 実装済み |
| FR-01204 | outputs/12-data-pipeline-sheriff/src/server.ts | outputs/12-data-pipeline-sheriff/src/server.test.ts | 実装済み |
...
システム役割は「data pipeline operations agent」。入力のmissing partitions・rerun safety・freshness impactを優先して、RERUN / HOLD DATA / ESCALATEの1つを選び、断定にはsummary・risks・evidenceを添える。実データにない事実を創作せず、不確実な場合は保守的なラベルと人間確認のactionを返す。JSON解析に失敗した場合はfallbackを返す。
...
既存の outputs/12-data-pipeline-sheriff/src/styles.css をデザインSSOTとし、semantic token、既存の色・間隔・フォーカス表示・レスポンシブブレークポイントを再利用する。新規色の追加は判断ラベルの意味が既存表現で不足する場合だけに限定する。

### 0.10 FR Dependency Graph

```
FR-01201 (input/contract) → FR-01202 (AI/execution) → FR-01203 (display)
                                      └──────────────→ FR-01204 (safety/operations)
```
...
Data Pipeline Sheriff によるmissing partitionsの自動判断を、判断可能なJSONと確認可能なUIとして提供する。

### 1.4 Goals / Non-Goals

**Goals**: missing partitions、rerun safety、freshness impactを構造化し、RERUN / HOLD DATA / ESCALATEと根拠・アクションを返す。Cloud Runで実行可能にする。
...
| FR-ID | 名称 | 要件 | 実装 | テスト |
|---|---|---|---|---|
| FR-01201 | 証拠入力 | target/context/signalsを受け付け、Zodでサイズ・形式を検証する。 | outputs/12-data-pipeline-sheriff/src/server.ts | outputs/12-data-pipeline-sheriff/src/server.test.ts |
| FR-01202 | AI判断 | data pipeline operations agentとしてmissing partitions・rerun safety・freshness impactをGeminiへ渡し、RERUN / HOLD DATA / ESCALATEのいずれかを返す。 | outputs/12-data-pipeline-sheriff/src/agent.ts | outputs/12-data-pipeline-sheriff/src/server.test.ts |
| FR-01203 | 結果表示 | confidence、summary、risks、actions、evidence、automationPlan、commentDraftを画面で確認できる。 | outputs/12-data-pipeline-sheriff/src/main.ts | outputs/12-data-pipeline-sheriff/src/server.test.ts |
| FR-01204 | 運用安全性 | Gemini失敗時のfallback、request ID、health/ready/version、認証・rate limit・構造化エラーを提供する。 | outputs/12-data-pipeline-sheriff/src/server.ts | outputs/12-data-pipeline-sheriff/src/server.test.ts |
