# 011-runbook-decay-detector: Runbook Decay Detector
...
| 用語 | 定義 | コード表現 |
|---|---|---|
| Agent | runbook freshness agentとして判断を行うAI役割 | agent |
| Decision | FRESH / PATCH DOCS / STALEの判断ラベル | decision |
...
| レイヤー | 実体 | 作業 | 条件 |
|---|---|:---:|---|
| Type/Schema | outputs/11-runbook-decay-detector/src/project.ts | 🔄 | 実装済み型・Zod契約 |
| Type/Schema | outputs/11-runbook-decay-detector/src/agent.ts | 🔄 | 実装済み型・Zod契約 |
| API/Service | outputs/11-runbook-decay-detector/src/server.ts | 🔄 | API/外部サービス境界 |
| UI | outputs/11-runbook-decay-detector/src/main.ts | 🔄 | UIまたは表示層 |
| Test | outputs/11-runbook-decay-detector/src/server.test.ts | ✅ | 回帰・契約テスト |

#### 0.1.1 Traceability Matrix

| FR-ID | 実装ファイル | テスト | 状態 |
|---|---|---|:---:|
| FR-01101 | outputs/11-runbook-decay-detector/src/server.ts | outputs/11-runbook-decay-detector/src/server.test.ts | 実装済み |
| FR-01102 | outputs/11-runbook-decay-detector/src/agent.ts | outputs/11-runbook-decay-detector/src/server.test.ts | 実装済み |
| FR-01103 | outputs/11-runbook-decay-detector/src/main.ts | outputs/11-runbook-decay-detector/src/server.test.ts | 実装済み |
| FR-01104 | outputs/11-runbook-decay-detector/src/server.ts | outputs/11-runbook-decay-detector/src/server.test.ts | 実装済み |
...
システム役割は「runbook freshness agent」。入力のcommand freshness・env var drift・service ownershipを優先して、FRESH / PATCH DOCS / STALEの1つを選び、断定にはsummary・risks・evidenceを添える。実データにない事実を創作せず、不確実な場合は保守的なラベルと人間確認のactionを返す。JSON解析に失敗した場合はfallbackを返す。
...
既存の outputs/11-runbook-decay-detector/src/styles.css をデザインSSOTとし、semantic token、既存の色・間隔・フォーカス表示・レスポンシブブレークポイントを再利用する。新規色の追加は判断ラベルの意味が既存表現で不足する場合だけに限定する。

### 0.10 FR Dependency Graph

```
FR-01101 (input/contract) → FR-01102 (AI/execution) → FR-01103 (display)
                                      └──────────────→ FR-01104 (safety/operations)
```
...
Runbook Decay Detector によるcommand freshnessの自動判断を、判断可能なJSONと確認可能なUIとして提供する。

### 1.4 Goals / Non-Goals

**Goals**: command freshness、env var drift、service ownershipを構造化し、FRESH / PATCH DOCS / STALEと根拠・アクションを返す。Cloud Runで実行可能にする。
...
| FR-ID | 名称 | 要件 | 実装 | テスト |
|---|---|---|---|---|
| FR-01101 | 証拠入力 | target/context/signalsを受け付け、Zodでサイズ・形式を検証する。 | outputs/11-runbook-decay-detector/src/server.ts | outputs/11-runbook-decay-detector/src/server.test.ts |
| FR-01102 | AI判断 | runbook freshness agentとしてcommand freshness・env var drift・service ownershipをGeminiへ渡し、FRESH / PATCH DOCS / STALEのいずれかを返す。 | outputs/11-runbook-decay-detector/src/agent.ts | outputs/11-runbook-decay-detector/src/server.test.ts |
| FR-01103 | 結果表示 | confidence、summary、risks、actions、evidence、automationPlan、commentDraftを画面で確認できる。 | outputs/11-runbook-decay-detector/src/main.ts | outputs/11-runbook-decay-detector/src/server.test.ts |
| FR-01104 | 運用安全性 | Gemini失敗時のfallback、request ID、health/ready/version、認証・rate limit・構造化エラーを提供する。 | outputs/11-runbook-decay-detector/src/server.ts | outputs/11-runbook-decay-detector/src/server.test.ts |
