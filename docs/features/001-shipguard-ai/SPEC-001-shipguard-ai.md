...
| 用語 | 定義 | コード表現 |
|---|---|---|
| Agent | AI release captainとして判断を行うAI役割 | agent |
| Decision | SHIP / WATCH / BLOCKの判断ラベル | decision |
...
| レイヤー | 実体 | 作業 | 条件 |
|---|---|:---:|---|
| Type/Schema | outputs/01-shipguard-ai/src/project.ts | 🔄 | 実装済み型・Zod契約 |
| Type/Schema | outputs/01-shipguard-ai/src/agent.ts | 🔄 | 実装済み型・Zod契約 |
| API/Service | outputs/01-shipguard-ai/src/server.ts | 🔄 | API/外部サービス境界 |
| UI | outputs/01-shipguard-ai/src/main.ts | 🔄 | UIまたは表示層 |
| Test | outputs/01-shipguard-ai/src/server.test.ts | ✅ | 回帰・契約テスト |

#### 0.1.1 Traceability Matrix

| FR-ID | 実装ファイル | テスト | 状態 |
|---|---|---|:---:|
| FR-00101 | outputs/01-shipguard-ai/src/server.ts | outputs/01-shipguard-ai/src/server.test.ts | 実装済み |
| FR-00102 | outputs/01-shipguard-ai/src/agent.ts | outputs/01-shipguard-ai/src/server.test.ts | 実装済み |
| FR-00103 | outputs/01-shipguard-ai/src/main.ts | outputs/01-shipguard-ai/src/server.test.ts | 実装済み |
| FR-00104 | outputs/01-shipguard-ai/src/server.ts | outputs/01-shipguard-ai/src/server.test.ts | 実装済み |
...
システム役割は「AI release captain」。入力のrelease verdict・CI failure clustering・Cloud Run regression signalsを優先して、SHIP / WATCH / BLOCKの1つを選び、断定にはsummary・risks・evidenceを添える。実データにない事実を創作せず、不確実な場合は保守的なラベルと人間確認のactionを返す。JSON解析に失敗した場合はfallbackを返す。
...
既存の outputs/01-shipguard-ai/src/styles.css をデザインSSOTとし、semantic token、既存の色・間隔・フォーカス表示・レスポンシブブレークポイントを再利用する。新規色の追加は判断ラベルの意味が既存表現で不足する場合だけに限定する。

### 0.10 FR Dependency Graph

```
FR-00101 (input/contract) → FR-00102 (AI/execution) → FR-00103 (display)
                                      └──────────────→ FR-00104 (safety/operations)
```
...
ShipGuard AI によるrelease verdictの自動判断を、判断可能なJSONと確認可能なUIとして提供する。

### 1.4 Goals / Non-Goals

**Goals**: release verdict、CI failure clustering、Cloud Run regression signalsを構造化し、SHIP / WATCH / BLOCKと根拠・アクションを返す。Cloud Runで実行可能にする。
...
| FR-ID | 名称 | 要件 | 実装 | テスト |
|---|---|---|---|---|
| FR-00101 | 証拠入力 | target/context/signalsを受け付け、Zodでサイズ・形式を検証する。 | outputs/01-shipguard-ai/src/server.ts | outputs/01-shipguard-ai/src/server.test.ts |
| FR-00102 | AI判断 | AI release captainとしてrelease verdict・CI failure clustering・Cloud Run regression signalsをGeminiへ渡し、SHIP / WATCH / BLOCKのいずれかを返す。 | outputs/01-shipguard-ai/src/agent.ts | outputs/01-shipguard-ai/src/server.test.ts |
| FR-00103 | 結果表示 | confidence、summary、risks、actions、evidence、automationPlan、commentDraftを画面で確認できる。 | outputs/01-shipguard-ai/src/main.ts | outputs/01-shipguard-ai/src/server.test.ts |
| FR-00104 | 運用安全性 | Gemini失敗時のfallback、request ID、health/ready/version、認証・rate limit・構造化エラーを提供する。 | outputs/01-shipguard-ai/src/server.ts | outputs/01-shipguard-ai/src/server.test.ts |
