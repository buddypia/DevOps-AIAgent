# 016-ai-exploratory-tester: AI Exploratory Tester
...
| 用語 | 定義 | コード表現 |
|---|---|---|
| Agent | exploratory QA agentとして判断を行うAI役割 | agent |
| Decision | TEST NOW / EXPLORE MORE / BLOCK RELEASEの判断ラベル | decision |
...
| レイヤー | 実体 | 作業 | 条件 |
|---|---|:---:|---|
| Type/Schema | outputs/16-ai-exploratory-tester/src/project.ts | 🔄 | 実装済み型・Zod契約 |
| Type/Schema | outputs/16-ai-exploratory-tester/src/agent.ts | 🔄 | 実装済み型・Zod契約 |
| API/Service | outputs/16-ai-exploratory-tester/src/server.ts | 🔄 | API/外部サービス境界 |
| UI | outputs/16-ai-exploratory-tester/src/main.ts | 🔄 | UIまたは表示層 |
| Test | outputs/16-ai-exploratory-tester/src/server.test.ts | ✅ | 回帰・契約テスト |

#### 0.1.1 Traceability Matrix

| FR-ID | 実装ファイル | テスト | 状態 |
|---|---|---|:---:|
| FR-01601 | outputs/16-ai-exploratory-tester/src/server.ts | outputs/16-ai-exploratory-tester/src/server.test.ts | 実装済み |
| FR-01602 | outputs/16-ai-exploratory-tester/src/agent.ts | outputs/16-ai-exploratory-tester/src/server.test.ts | 実装済み |
| FR-01603 | outputs/16-ai-exploratory-tester/src/main.ts | outputs/16-ai-exploratory-tester/src/server.test.ts | 実装済み |
| FR-01604 | outputs/16-ai-exploratory-tester/src/server.ts | outputs/16-ai-exploratory-tester/src/server.test.ts | 実装済み |
...
システム役割は「exploratory QA agent」。入力のexploration route・bug reproduction・risk-based coverageを優先して、TEST NOW / EXPLORE MORE / BLOCK RELEASEの1つを選び、断定にはsummary・risks・evidenceを添える。実データにない事実を創作せず、不確実な場合は保守的なラベルと人間確認のactionを返す。JSON解析に失敗した場合はfallbackを返す。
...
既存の outputs/16-ai-exploratory-tester/src/styles.css をデザインSSOTとし、semantic token、既存の色・間隔・フォーカス表示・レスポンシブブレークポイントを再利用する。新規色の追加は判断ラベルの意味が既存表現で不足する場合だけに限定する。

### 0.10 FR Dependency Graph

```
FR-01601 (input/contract) → FR-01602 (AI/execution) → FR-01603 (display)
                                      └──────────────→ FR-01604 (safety/operations)
```
...
AI Exploratory Tester によるexploration routeの自動判断を、判断可能なJSONと確認可能なUIとして提供する。

### 1.4 Goals / Non-Goals

**Goals**: exploration route、bug reproduction、risk-based coverageを構造化し、TEST NOW / EXPLORE MORE / BLOCK RELEASEと根拠・アクションを返す。Cloud Runで実行可能にする。
...
| FR-ID | 名称 | 要件 | 実装 | テスト |
|---|---|---|---|---|
| FR-01601 | 証拠入力 | target/context/signalsを受け付け、Zodでサイズ・形式を検証する。 | outputs/16-ai-exploratory-tester/src/server.ts | outputs/16-ai-exploratory-tester/src/server.test.ts |
| FR-01602 | AI判断 | exploratory QA agentとしてexploration route・bug reproduction・risk-based coverageをGeminiへ渡し、TEST NOW / EXPLORE MORE / BLOCK RELEASEのいずれかを返す。 | outputs/16-ai-exploratory-tester/src/agent.ts | outputs/16-ai-exploratory-tester/src/server.test.ts |
| FR-01603 | 結果表示 | confidence、summary、risks、actions、evidence、automationPlan、commentDraftを画面で確認できる。 | outputs/16-ai-exploratory-tester/src/main.ts | outputs/16-ai-exploratory-tester/src/server.test.ts |
| FR-01604 | 運用安全性 | Gemini失敗時のfallback、request ID、health/ready/version、認証・rate limit・構造化エラーを提供する。 | outputs/16-ai-exploratory-tester/src/server.ts | outputs/16-ai-exploratory-tester/src/server.test.ts |
