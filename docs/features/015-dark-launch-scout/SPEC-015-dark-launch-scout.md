# 015-dark-launch-scout: Dark Launch Scout
...
| 用語 | 定義 | コード表現 |
|---|---|---|
| Agent | dark launch readiness scoutとして判断を行うAI役割 | agent |
| Decision | LAUNCH / KEEP DARK / DISABLEの判断ラベル | decision |
...
| レイヤー | 実体 | 作業 | 条件 |
|---|---|:---:|---|
| Type/Schema | outputs/15-dark-launch-scout/src/project.ts | 🔄 | 実装済み型・Zod契約 |
| Type/Schema | outputs/15-dark-launch-scout/src/agent.ts | 🔄 | 実装済み型・Zod契約 |
| API/Service | outputs/15-dark-launch-scout/src/server.ts | 🔄 | API/外部サービス境界 |
| UI | outputs/15-dark-launch-scout/src/main.ts | 🔄 | UIまたは表示層 |
| Test | outputs/15-dark-launch-scout/src/server.test.ts | ✅ | 回帰・契約テスト |

#### 0.1.1 Traceability Matrix

| FR-ID | 実装ファイル | テスト | 状態 |
|---|---|---|:---:|
| FR-01501 | outputs/15-dark-launch-scout/src/server.ts | outputs/15-dark-launch-scout/src/server.test.ts | 実装済み |
| FR-01502 | outputs/15-dark-launch-scout/src/agent.ts | outputs/15-dark-launch-scout/src/server.test.ts | 実装済み |
| FR-01503 | outputs/15-dark-launch-scout/src/main.ts | outputs/15-dark-launch-scout/src/server.test.ts | 実装済み |
| FR-01504 | outputs/15-dark-launch-scout/src/server.ts | outputs/15-dark-launch-scout/src/server.test.ts | 実装済み |
...
システム役割は「dark launch readiness scout」。入力のhidden traffic・flag readiness・beta safetyを優先して、LAUNCH / KEEP DARK / DISABLEの1つを選び、断定にはsummary・risks・evidenceを添える。実データにない事実を創作せず、不確実な場合は保守的なラベルと人間確認のactionを返す。JSON解析に失敗した場合はfallbackを返す。
...
既存の outputs/15-dark-launch-scout/src/styles.css をデザインSSOTとし、semantic token、既存の色・間隔・フォーカス表示・レスポンシブブレークポイントを再利用する。新規色の追加は判断ラベルの意味が既存表現で不足する場合だけに限定する。

### 0.10 FR Dependency Graph

```
FR-01501 (input/contract) → FR-01502 (AI/execution) → FR-01503 (display)
                                      └──────────────→ FR-01504 (safety/operations)
```
...
Dark Launch Scout によるhidden trafficの自動判断を、判断可能なJSONと確認可能なUIとして提供する。

### 1.4 Goals / Non-Goals

**Goals**: hidden traffic、flag readiness、beta safetyを構造化し、LAUNCH / KEEP DARK / DISABLEと根拠・アクションを返す。Cloud Runで実行可能にする。
...
| FR-ID | 名称 | 要件 | 実装 | テスト |
|---|---|---|---|---|
| FR-01501 | 証拠入力 | target/context/signalsを受け付け、Zodでサイズ・形式を検証する。 | outputs/15-dark-launch-scout/src/server.ts | outputs/15-dark-launch-scout/src/server.test.ts |
| FR-01502 | AI判断 | dark launch readiness scoutとしてhidden traffic・flag readiness・beta safetyをGeminiへ渡し、LAUNCH / KEEP DARK / DISABLEのいずれかを返す。 | outputs/15-dark-launch-scout/src/agent.ts | outputs/15-dark-launch-scout/src/server.test.ts |
| FR-01503 | 結果表示 | confidence、summary、risks、actions、evidence、automationPlan、commentDraftを画面で確認できる。 | outputs/15-dark-launch-scout/src/main.ts | outputs/15-dark-launch-scout/src/server.test.ts |
| FR-01504 | 運用安全性 | Gemini失敗時のfallback、request ID、health/ready/version、認証・rate limit・構造化エラーを提供する。 | outputs/15-dark-launch-scout/src/server.ts | outputs/15-dark-launch-scout/src/server.test.ts |
