# 013-rollback-concierge: Rollback Concierge
...
| 用語 | 定義 | コード表現 |
|---|---|---|
| Agent | rollback sequencing agentとして判断を行うAI役割 | agent |
| Decision | ROLLBACK READY / MANUAL CHECK / DO NOT ROLLBACKの判断ラベル | decision |
...
| レイヤー | 実体 | 作業 | 条件 |
|---|---|:---:|---|
| Type/Schema | outputs/13-rollback-concierge/src/project.ts | 🔄 | 実装済み型・Zod契約 |
| Type/Schema | outputs/13-rollback-concierge/src/agent.ts | 🔄 | 実装済み型・Zod契約 |
| API/Service | outputs/13-rollback-concierge/src/server.ts | 🔄 | API/外部サービス境界 |
| UI | outputs/13-rollback-concierge/src/main.ts | 🔄 | UIまたは表示層 |
| Test | outputs/13-rollback-concierge/src/server.test.ts | ✅ | 回帰・契約テスト |

#### 0.1.1 Traceability Matrix

| FR-ID | 実装ファイル | テスト | 状態 |
|---|---|---|:---:|
| FR-01301 | outputs/13-rollback-concierge/src/server.ts | outputs/13-rollback-concierge/src/server.test.ts | 実装済み |
| FR-01302 | outputs/13-rollback-concierge/src/agent.ts | outputs/13-rollback-concierge/src/server.test.ts | 実装済み |
| FR-01303 | outputs/13-rollback-concierge/src/main.ts | outputs/13-rollback-concierge/src/server.test.ts | 実装済み |
| FR-01304 | outputs/13-rollback-concierge/src/server.ts | outputs/13-rollback-concierge/src/server.test.ts | 実装済み |
...
システム役割は「rollback sequencing agent」。入力のrevision rollback・data compatibility・sequence safetyを優先して、ROLLBACK READY / MANUAL CHECK / DO NOT ROLLBACKの1つを選び、断定にはsummary・risks・evidenceを添える。実データにない事実を創作せず、不確実な場合は保守的なラベルと人間確認のactionを返す。JSON解析に失敗した場合はfallbackを返す。
...
既存の outputs/13-rollback-concierge/src/styles.css をデザインSSOTとし、semantic token、既存の色・間隔・フォーカス表示・レスポンシブブレークポイントを再利用する。新規色の追加は判断ラベルの意味が既存表現で不足する場合だけに限定する。

### 0.10 FR Dependency Graph

```
FR-01301 (input/contract) → FR-01302 (AI/execution) → FR-01303 (display)
                                      └──────────────→ FR-01304 (safety/operations)
```
...
Rollback Concierge によるrevision rollbackの自動判断を、判断可能なJSONと確認可能なUIとして提供する。

### 1.4 Goals / Non-Goals

**Goals**: revision rollback、data compatibility、sequence safetyを構造化し、ROLLBACK READY / MANUAL CHECK / DO NOT ROLLBACKと根拠・アクションを返す。Cloud Runで実行可能にする。
...
| FR-ID | 名称 | 要件 | 実装 | テスト |
|---|---|---|---|---|
| FR-01301 | 証拠入力 | target/context/signalsを受け付け、Zodでサイズ・形式を検証する。 | outputs/13-rollback-concierge/src/server.ts | outputs/13-rollback-concierge/src/server.test.ts |
| FR-01302 | AI判断 | rollback sequencing agentとしてrevision rollback・data compatibility・sequence safetyをGeminiへ渡し、ROLLBACK READY / MANUAL CHECK / DO NOT ROLLBACKのいずれかを返す。 | outputs/13-rollback-concierge/src/agent.ts | outputs/13-rollback-concierge/src/server.test.ts |
| FR-01303 | 結果表示 | confidence、summary、risks、actions、evidence、automationPlan、commentDraftを画面で確認できる。 | outputs/13-rollback-concierge/src/main.ts | outputs/13-rollback-concierge/src/server.test.ts |
| FR-01304 | 運用安全性 | Gemini失敗時のfallback、request ID、health/ready/version、認証・rate limit・構造化エラーを提供する。 | outputs/13-rollback-concierge/src/server.ts | outputs/13-rollback-concierge/src/server.test.ts |
