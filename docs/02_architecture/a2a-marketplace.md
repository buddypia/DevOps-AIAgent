...
- Runbook: `DRY_RUN=1 scripts/bootstrap_github_actions_deploy.sh`、`scripts/bootstrap_github_actions_deploy.sh`、GitHub Actions `deploy-cloud-run.yml` 手動実行、`gcloud auth login`、Cloud Build submit、Agent Card skill count、MVP Readiness、Recording Script、Pilot Value、Acceptance Matrix、A2A artifactの再確認コマンドを返す
