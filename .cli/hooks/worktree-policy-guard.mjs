...
/**
 * 현재 branch 가 worktree 정책 보호 대상(직접 편집 차단)인지 판정.
 * policy.protected_branches 가 명시되면 그것을, 없으면 default ['main', 'master'] 사용.
 * 외부 이식 프로젝트가 master 기본이어도 가드가 동작하도록 함 (worktree-new 의
 * base 자동 감지와 정합 — 하드코딩 'main' 가정이 master repo 에서 no-op 되던 함정 해소).
 */
...
function buildDenyMessage(relPath, branch = 'main') {
  return (
    `[Worktree Policy] ${branch} 브랜치 직접 편집 차단: ${relPath}\n\n` +
    `이 파일은 worktree 에서 작업해야 합니다.\n` +
    `  → worktree 생성 (표준 진입점, fetch + ff + 최신 base 기준):\n` +
...
export async function run(data) {
  try {
    ...
    // 보호 branch (default main+master, policy.protected_branches 로 재정의) 외엔 통과.
