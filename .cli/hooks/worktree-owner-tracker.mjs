 ...
 *
 * Multi-CLI: `session_id` 는 Claude Code 와 Codex (PostToolUse 공통 필드 — 공식 문서
 * https://developers.openai.com/codex/hooks) payload 에 존재한다. 따라서 `run(data)` 를
 * export 하여 단일 dispatcher (`.cli/_cli-dispatch.mjs`) 가 Codex payload 정규화
 * 후 위임한다 (가드와 동일한 dispatcher 위임 — MULTI-CLI.md). Antigravity payload 는 session_id 필드
 * 부재 확정(2026-06-20 검증: transcriptPath 로 식별) — cd-into-worktree 모델이라 Layer 1 충분,
 * Layer 2 사실상 no-op(R-CM-036).
 *
 ...
 *
 * Claude Code standalone (아래 safeHookMain) + Codex/Antigravity dispatcher 위임이 공유한다.
 * dispatcher 는 `cli-adapter-utils.mjs#runAdapter` 로 CLI payload 를 Claude 형식
...
// 예약 플래그로, dispatcher 의 동적 import 시점에는 설정되지 않는다 → 이 블록은
// dispatcher import 부수효과로도 발동한다 (기존 guard 본체와 동일 구조).
