 ...
 * Hook が命令文字列を検査する際、heredoc body 内の trigger string まで
 * false-positive として遮断してしまう問題を root cause から遮断する。
 *
 * 対応する変形:
 *   `<<EOF`         — 基本
 *   `<<-EOF`        — tab strip 変形
 ...
 * standalone 実行時にトリガーされ、直接 import すると stdout が衝突する。本 lib に分離した。
 */
...
/**
 * 命令から heredoc body をすべて除去し、inspection-safe な string を返す。
 *
...
export function stripHeredocBodies(command) {
  ...
  // Fast-path — heredoc opener `<<` が無い場合は regex に入らない。PreToolUse Bash hook の
  // 95%以上のケース（単純な git/ls/npm）に heredoc は無い。.includes scan の方が regex の
  // [\s\S]*? backtracking コストよりはるかに安い。
