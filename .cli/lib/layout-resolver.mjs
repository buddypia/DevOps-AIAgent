...
/**
 * 임의 경로가 brief2dev 파이프라인을 사용하는 프로젝트(`.brief2dev/` 마커 보유)인지 확인한다.
 * resolveSystemPersistentRoot() 등 다른 함수와 달리 **현재 프로세스 자신의 root 가 아닌 임의
 * 외부 경로**(예: 다른 worktree, transplant-bundle 이식 대상 프로젝트)를 검사할 때 사용한다 —
 * 예: create-pr/ops.mjs 가 다른 worktree 의 run 산출물 보존 후보를 판단할 때.
 * @param {string} root - 검사할 절대/상대 경로
 * @returns {boolean}
 */
