 ...
 *
 * data/registry/brief2dev-layout.json のカテゴリを決定的パスに解決する。
 * すべての hook/script は .brief2dev/ を直接ハードコーディングせず、本モジュールのヘルパーを使用すべき。
 *
 * 役割:
 ...
 *   - active run_id 照会（.brief2dev/run/active.json 優先）
 *
 * 設計原則:
...
/** プロジェクトルート（CLAUDE_PROJECT_DIR 優先）— プロセスの実際のルート（system/governance/scan 基準、不変） */
const PROJECT_DIR = process.env.CLAUDE_PROJECT_DIR
  ? resolve(process.env.CLAUDE_PROJECT_DIR)
  : process.cwd();

/**
 * worktree_local 解決 base の同期スコープ override（R-CM-035 例外5 — loom cross-worktree run mutation）。
 *
 * webui サーバー（main プロセス）が *別の worktree* の run を archive/削除する際、worktree_local
 * 資産（`run/active.json`, `runs/<id>/`）をその worktree 基準で解決させる。system_persistent
 * （`getArchivesRoot`/`resolveSystemFile` → `resolveSystemPersistentRoot` git-common-dir）は影響を
 * 受けないため、archive snapshot/registry/index は常に main に蓄積される（R-CM-026 / R-CM-030 整合）。
 *
 * **同期専用**: `fn` は同期関数でなければならない（try/finally で復元）。async 呼び出しの間に override が
 * 漏洩しないよう、archive コアは sync fs のみを使用する。webui は単一プロセス・単一ユーザーのため
 * AsyncLocalStorage なしで module 変数 + sync スコープで十分（over-engineering 回避）。
 */
let _projectDirOverride = null;

/**
 * worktree_local 解決 base（override > PROJECT_DIR）。getPipelineDataRoot + saga state dir などで使用。
 * system_persistent 解決（resolveSystemPersistentRoot）は本関数を使わないため override は無影響。
 * @returns {string} 絶対パス
 */
...
/**
 * `dir` を worktree_local base として override した状態で同期関数 `fn` を実行し、結果を返す。
 * 実行後（throw 含む）は必ず以前の override に復元する（re-entrant 安全 — 以前の値を保存）。
 *
 * @template T
 * @param {string} dir - worktree 絶対/相対パス。falsy なら override 解除効果。
 * @param {() => T} fn - 同期コールバック
 * @returns {T}
 */
 ...
 *
 * 戻り値は `.brief2dev` 親ディレクトリのパス（絶対パス）。呼び出し側が join で
 * `.brief2dev/system/...` を組み立てる。
 *
 * **落とし穴注意**: 名前は "system_persistent root" だが *system ファイルのパスではない*。
 * `<root>` のみを返すため、呼び出し側が `.brief2dev/system/<filename>` を組み立てる必要がある。
 * system ファイルへの直接アクセスは `resolveSystemFile(filename)` を推奨 — 自動的に
 * `.brief2dev/system/<filename>` の絶対パスを算出 + new vs legacy path fallback を処理。
 * raw root への直接 join（例: `join(root, 'learnings.jsonl')`）は main root にファイルを
 * 誤って append してしまう違反パターン（本セッション 2026-05-24 の事例）。
 *
 * 本関数は throw しない — git 失敗は fallback で graceful degrade する。
 *
 * @param {object} [opts]
...
export function resolveSystemPersistentRoot(opts = {}) {
  ...
  // R-CM-030: vitest sandbox 隔離。B2D_TEST_SANDBOX=1 の場合 CLAUDE_PROJECT_DIR
  // を system_persistent root として使用（git common-dir を無視）。worktree で
  // vitest 実行時にも main repo の system を変更しないよう強制する。
 ...
 * 外部パス**（例: 別の worktree、transplant-bundle 移植先プロジェクト）を検査する際に使用する —
 * 例: create-pr/ops.mjs が別の worktree の run 成果物の保存候補を判断する時。
...
/**
 * ランタイムデータルート（例: `.brief2dev/` または成果物の `docs/discovery/`）— env-aware + project-config-aware（lazy lookup）。
 *
 * 優先順位（呼び出しごとに評価）:
 *   1. <PROJECT_DIR>/project-config.json#pipeline_data_root 明示 string → join(PROJECT_DIR, その path)
 *   2. fallback → join(PROJECT_DIR, '.brief2dev')
 *
 * **Lazy lookup（ユーザー決定 2026-05-25 オプション3）**: const ではなく関数 — 呼び出しごとに project-config.json を
 * sync read。R-CM-026 Rule 8 単一エントリポイント SSOT: 本モジュールのみ `.brief2dev` literal を保有、pipeline-config など
 * 他のモジュールは本関数を import して委譲。
 *
 * **セキュリティガード**: pipeline_data_root 入力に絶対パス（`/...`）または path traversal（`..`）を含む場合 silent fallback
 * （R-CM-006 Rule 2 fail-open 整合）。成果物の project-config.json は scaffold-deploy が生成するためユーザー直接
 * 制御 input ではないが defense-in-depth。
 *
 * **パフォーマンス**: 呼び出しごとに disk I/O。hot loop で呼び出す場合は caller が結果を変数に cache することを推奨。
 *
 * @returns {string} 絶対パス
 */
export function getPipelineDataRoot() {
  // worktree_local base — withProjectDirOverride 適用時はその worktree 基準（R-CM-035 例外5）。
  ...
  if (existsSync(configPath)) {
    try {
      ...
      if (typeof raw === 'string') {
        ...
        // `..` substring 検査のため `docs..v2` のような任意の名前も拒否 — `pipeline_data_root` は単純なディレクトリ名（例: `docs/discovery`）を想定。
        if (trimmed.length > 0 && !trimmed.startsWith('/') && !trimmed.includes('..')) {
          return join(base, trimmed);
        }
      }
    } catch {
      // JSON parse 失敗など silent fallback
    }
  }
...
/**
 * `.brief2dev/runs/` — run-scoped 成果物ルート。
 *
 * @returns {string} 絶対パス
 */
 ...
 * 成果物が main に sync され、cross-aidea archive の蓄積が保存されるようにする。`.brief2dev/run/active.json` は
 * worktree-local（per-session 隔離）だが archives は cross-worktree 共有。
 *
 * 本関数は getPipelineDataRoot() lookup とは *独立* — system_persistent root が base のため project-config.json
 * の `pipeline_data_root` の影響なし。成果物（観点2）では意味を持たない。**本体（観点1）専用の cross-aidea 資産**。
 *
 * @returns {string} 絶対パス
 */
 ...
 *   1. projectDir 明示 + 明示 root に新パス存在 → 明示 root の新パス（テスト隔離優先）
 *   2. system_persistent root（`git common-dir` の親）の新パス存在 → そのパス
 *   3. system_persistent root の旧パス（.brief2dev/<filename>）存在 + 新パス不在 → 旧パス（P3 legacy fallback）
 *   4. 両方不在の場合: 新パス（write target）
 *
 * projectDir 引数はテスト隔離用 — デフォルト値 PROJECT_DIR の場合 system_persistent root を自動解決。
 * 明示時はそのパスを強制使用（BRIEF2DEV_SYSTEM_ROOT env と同等の効果）。
 *
 * @param {string} filename - ファイル名（例: "learnings.jsonl", "registry.json"）
 * @param {string} [projectDir] - プロジェクトルート明示（テスト隔離）。未指定時は system_persistent root。
 *   **注意**: `filename === 'active-run.json'` の場合、本引数は無視され `getActiveRunPath()` に
 *   委譲される（worktree_local lifecycle、R-CM-026 2026-05-14）。active-run state の worktree 隔離が
 *   必要な呼び出し側は `getActiveRunPath()` を直接使用することを推奨。
 * @returns {string} 絶対パス
 */
export function resolveSystemFile(filename, projectDir) {
  // active-run.json は worktree_local として分離されているため（R-CM-026 `run` カテゴリ）、
  // 呼び出し側のコード変更なしに自動委譲。projectDir 明示は無視 — getActiveRunPath が
  // PROJECT_DIR（worktree-local）を自動解決。
 ...
 * worktree ごとに隔離（multi-session safe）。main 共有 system 資産（pipeline-memory
 * など）と異なり PROJECT_DIR 基準の `.brief2dev/run/active.json` で解決する。
 *
 * Legacy fallback（段階的マイグレーション + test fixture 互換）:
 *   1. `<wt>/.brief2dev/run/active.json` 存在 → そのパス
 *   2. system_persistent root の `system/active-run.json` 存在 → そのパス（P3 legacy）
 *   3. system_persistent root の `.brief2dev/active-run.json` 平坦位置に存在 → そのパス（P3 flat）
 ...
 * active-run は worktree_local（R-CM-026 `run` カテゴリ、ユーザー決定 2026-05-14）のため
 * cross-worktree running guard は各 worktree の active を個別に確認する必要がある。
 * パスの存在有無は呼び出し側が判断する（存在に関係なく候補を返す — fail-open）。
 *
 * @param {string} [projectDir=PROJECT_DIR] プロジェクトルート
 * @returns {string[]} active-run.json 絶対パス一覧（main 1 + worktree N）
 */
export function listAllActiveRunPaths(projectDir = PROJECT_DIR) {
  ...
  // 誤認したり worktree 内部（node_modules/src など）を over-scan するのを防ぐ。
  let level1 = [];
  try {
  ...
  for (const entry of level1) {
    ...
    // feature/foo, fix/bar のような2階層ブランチは1レベル追加探索（ディレクトリのみ）
    let level2 = [];
    try {
...
/**
 * active-run.json の run_id を返す（idle/なければ null）。
 *
 * @returns {string|null}
 */
...
/**
 * run-scoped カテゴリのディレクトリ（特定 run_id 明示）。
 *
 ...
 * カテゴリ sub-dir ではなく run-level 資産（inbox-manifest.json など）の位置決定に使用。
 *
 * 優先順位:
 ...
 * R-CM-028 Two-Perspective Boundary データ分離を適用 — system pipeline-memory.json と隔離された
 ...
 *   2. runId 未指定 + active run_id あり → active run の idea-memory
 *   3. runId 未指定 + active なし → null（idle 状態では idea-memory 不在）
 *
 * @param {string} [runId] - 特定の run_id 明示。null または undefined 時は active を使用。
 * @returns {string|null} 絶対パスまたは null（idle + runId 未指定時）
 */
 ...
 *
 * @param {string} subdir - "handoff" | "retrospectives" | "audits" | その他 governance category
 ...
 *
 * @param {string} subdir - archive 内部サブディレクトリ
 * @param {string} [projectDir=PROJECT_DIR] - プロジェクトルートのオーバーライド（テスト隔離用）
 * @returns {string} 絶対パス
 */
